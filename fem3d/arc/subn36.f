c
c $Id: subn36.f,v 1.14 2001/12/07 02:45:37 georg Exp $
c
c closing routines
c
c contents :
...
c			/bnd/ substituted by utility routine
...
c local
	...
	integer k1,k2,k,l
	real scalo,zin,zout,u,v,uvref2,dx,dy
...
c
	...
	do while (iact.gt.0.and.it.ge.ivccv(jitb+iact)
     +			.and.ivccv(jitb+iact).ne.-999)
		...
		iact=iact+2
		if(iact.gt.nitb) iact=0		!no more data to read
...
c	!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
	...
	write(nb13,'(1x,a,4e10.3)') 'scal,scalo,zin,zout :'
     +				,scal,scalo,zin,zout
	write(nb13,*)
c	!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
c
	  ...
	  if(bspecl.or.bflxcl) then
            ...
            do i=2,nkboc
              ...
              if(istp.eq.-isoft.or.bimm) then
                ...
                uv1=(-u1*dy+v1*dx)/l
                uv2=(-u2*dy+v2*dx)/l
                flux1=float(idt)*l*(2.*uv1+uv2)/6.
                flux2=float(idt)*l*(uv1+2.*uv2)/6.
...
c             !&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
c              write(nb13,*) 'geyer :',geyer
c              write(nb13,'(1x,a,4e10.3)') 'hm,rqv(k1),rqv(k2) :'
c     +           ,hm,rqv(k1),rqv(k2)
c             !&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
            end do
...
c
          if(bfalse) then  !$$1 - never touch depth values !$$ALPHA
           do i=1,niboc
		   ie=ivccv(jiboc+i)
		   do ii=1,3
		      kn=nen3v(ii,ie)
...
c	   nodes of boundary
           ...
           if(ibnd.gt.0.and.nkboc.gt.0) then
             ...
             write(6,*) '   ibnd and kboc are mutually esclusive'
             bstop=.true.
