...
C****************************************************************************
     ...
     @RLENGTH,QS,QSDIR,SEDM,SED,SEDDIR,FALL,C0A,EDRATE,ZS,TAOS,
...
C AND THE SUSPENDED SEDIMENT TRANSPORT RATE AND DIRECTION
     ...
     @USTBF,Z0,GD,DX,RHOS,QS,QSDIR,C0A)
...
C---------------------------------------------------------------------------
      ...
      SUBROUTINE FRICFAC(D,UZ,Z,CDIR,UB,PER,WDIR,AB,GD,VISK,RHOW,RHOS,
     @Z0C,PHIB,PHI100,DELTACW,USTCS,USTWS,USTCWSB,USTC,USTW,RPLCOEF,
     @USTBF,USTCRB,USTUP,Z0,FCW,UA,U100,USTCWS,USTCW,RHINP,RLINP)
...
C INITIALIZE PARAMETERS
      ...
      COHES = GD.LE.0.000063		!true=cohesive; false=non cohesive
...
c        IF (USTCS .LT. USTCRB .OR. GD .GE. 0.0005 .OR. COHES) THEN
        IF ( USTCS .LT. USTCRB .OR. GD .GE. 0.005 .OR. COHES) THEN	!ccf
           RHEIGHT=RHINP
           RLENGTH=RLINP
        ELSE
...
C OBTAIN THE USTCS CONVERGENCE CRITERION DELTA
        ...
        if (USTCS .GE. USTUP) delta=0.00001             !ccf
        if (USTCSP .LT. USTCS) delta=0.00001            !ccf
...
c      IF(USTCWSE .GE. USTCRB .AND. USTCWS .LT. USTCRB) THEN
...
C CALCULATE REFERENCE CONCENTRATIONS C0 AT Z0S
          ...
          Z0S = MAX(Z0,Z0S)
          C0=GETC0(USTCS,USTUP,TAOST,USTBF)
c          C0=GETC01(GD,DX,Z0,TAOST,D)*RHOS
          DO 40 I=1,IL
...
C CALCULATE REFERENCE CONCENTRATIONS C0 AT Z0S
            ...
            Z0S = (26.3*TAOCRB*TAOST)/(RHOW*G*(S-1)) + GD/12.
            Z0S = MAX(Z0,Z0S)
            C0=GETC0(USTWS,USTUP,TAOST,USTBF)
c            C0=GETC01(GD,DX,Z0,TAOST,D)*RHOS
            DO 60 I=1,IL
...
c          IF (USTCWSB.LT.USTCRS) THEN
          IF (USTCWS.LT.USTCRS) THEN	!ccf
            QS=0
            GOTO 300
          ELSE
C CALCULATE REFERENCE CONCENTRATIONS C0 AT Z0S AND CZD AT DELTACW
            ...
            Z0S = (26.3*TAOCRB*TAOST)/(RHOW*G*(S-1)) + GD/12.
            Z0S = MAX(Z0,Z0S)
            C0=GETC0(USTCWS,USTUP,TAOST,USTBF)		!ccf
c            C0=GETC0(USTCWSB,USTUP,TAOST,USTBF)
c            C0=GETC01(GD,DX,Z0,TAOST,D)*RHOS
...
C PREDICT SUSPENSION PROFILE BASED ON THE MODIFIED ROUSE EQUATION
            DO 90 I=1,IL
              ...
              ELSE
                CZ(I)=CZD*(ZZ(I)/DELTACW)**(-0.74*FALL/(0.4*USTCW))
              ENDIF
              IF (CZ(I) .LT. 0.) CZ(I) = 0.
 90         CONTINUE
          ENDIF
...
C*********************************************************************
 ...
 95   CONTINUE
      C0A = SOM/D
!      C0A=C0*((D/2)/Z0S)**(-0.74*FALL/(0.4*USTC))
...
C FUNCTION GETC0
C ************************************************************
C This function compute the reference concentration C0 using 
C Smith and McLean (1977).
C     UST = SHEAR VELOCITY
C  GAMMA0 = SAND RESUSPENSION COEFFICIENT
C      CB = BULK BOTTOM SEDIMENT CONCENTRATION (KG/M^3)
C   USTBF = CRITICAL SHEAR VELOCITY FOR RIPPLE BREAKOFF
C---------------------------------------------------------------------------

      FUNCTION GETC0(UST,USTUP,TAOST,USTBF)
...
C---------------------------------------------------------------------------
      ...
      FUNCTION GETC01(GD,DX,Z0,TAOST,D)
      ...
      DOUBLE PRECISION TAOST    !NORMALIZED EXCESS SHEAR STRESS
      DOUBLE PRECISION D        !WATER DEPTH (M)
      DOUBLE PRECISION ZZ0	!AUX

      ZZ0 = Z0*30. / 2.
      ZZ0 = MAX(ZZ0,0.01*D)

      GETC01=(0.015*GD*TAOST**1.5)/(ZZ0*DX**0.3)
...
C Do 2 iterations do define the eroded depth, for the firt RE is calculated
...
C higher than the previous bed surface.
         BEDCHA(1,2)=MIN(TAU0*1.1d0,BEDCHA(1,2))
...
C If the computed TAUCE is smaller than the previous one, keep the previous one
     ...
     &         (1+TEROC*(1-EXP(TEROD*ABOVE(IB))))
         BEDCHA(IB,2)=MAX(BEDCHA(IB,2),TAUCE)
