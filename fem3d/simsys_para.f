...
! allocates data structure
	...
	call shympi_barrier
        ...
        write(6,*) 'using Paralution routines ',my_id,nkn
        write(6,*) '----------------------------------------'
	...
	call para_initialize_system(l_matrix)		!calls coo_init_new
...
! must be called before every assembly of matrix
	...
	if( bsysexpl ) then
	  ...
	  if( bmpi ) then
	    call para_init_system(g_matrix)
	  end if
	  call para_init_system(l_matrix)
	end if
...
!---------------------------------------------------------------
	...
	call para_solve_system_csr(g_matrix,.false.,n2max,nkn,zglobal)
...
! solves system - z is used for initial guess, not the final solution
	...
	if( bsysexpl ) then			!explicit - solved direct
	  ...
	  n2max = mm%n2max
	  call para_solve_system_csr(l_matrix,.false.,n2max,n,z)
	end if
...
! solves system - z is used for initial guess, not the final solution
	...
	nn = n*(nlv + 2) !DEB
	call para_solve_system_csr(l_matrix,.true.,n3max,nn,p) !DEB
