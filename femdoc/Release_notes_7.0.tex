...
strong that they are not backward compatible with the older versions. This means that changes have to made to the
...
old input files to the new file format. This routine is quite 
...
file are kept to some obligatory parameters and the change
in specifying space varying boundary coordinates. All this
...
The file formats
used for input files were (to say it mildly) really sub-optimal. 
For every variable, file formats changed, as well as the formats
were different from boundary conditions, initial values,
...
To end this chaotic situation, in version 7 of the model
it has been decided to end this chaos and standardize
...
parameter |iwtype|. A part from the first time column, the second and third can be wind velocity in $x,y$ direction, or it can be
...
All of the above usages already were covered in older versions
of \shyfem{}. However, the file formats between the different
applications were different. It is important for code
...
of file conversion. Thw following list shows the extra information contained in the FEM files and explains why it is needed:
\begin{itemize}
...
\subsection{New time management}
...
time in FEM files because it makes the date and time immediately
readable.
