...
!
! !INTERFACE:
   subroutine adv_center_bfm(N,dt,h,ho,ww,Bcup,Bcdw,Yup,Ydw,methodx,Y)
!
! !DESCRIPTION:
!
...
!  MODIFIED by Piet Ruardij for use with BFM
!
...
!BOC
   ...
   method=methodx
   if ( methodx >= 10 ) method = methodx-10;
...
!     vertical flow velocity and for settling of suspended matter.
      c=1.0*dt/float(it)
      if ( method .eq. BIOSTREAM ) then
        do k=1,N
           Y(k)=Y(k)-c/h(k) *  ((cu(k)-cu(k-1)))  
        enddo
      else 
        do k=1,N
           Y(k)=Y(k)-c/h(k) *  ((cu(k)-cu(k-1))        &
...
#endif
   if ( methodx >= 10 ) then
      if ( abs(s- sum(h*y)) > 1.0D-6 ) then
         write(*,*) ' No balance in adv_center'
      endif
   endif

   return
   end subroutine adv_center_bfm
!EOC
