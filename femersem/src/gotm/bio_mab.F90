...
!BOP
!
! !MODULE: bio_mab --- IOW model ERGOM, MaBenE version \label{sec:bio-mab}
!
! !INTERFACE:
   module bio_mab
!
...
! !PUBLIC MEMBER FUNCTIONS:
   public init_bio_mab, init_var_mab, var_info_mab, &
          surface_fluxes_mab,light_mab, do_bio_mab, end_bio_mab
!
...
!
! !LOCAL VARIABLES:
   namelist /bio_mab_nml/ numc,p1_initial,p2_initial,p3_initial,zo_initial,  &
                      de_initial,am_initial,ni_initial,po_initial,           &
                      o2_initial,se_initial,sfl_po,sfl_am,sfl_ni,            &
                      surface_flux_method,   &
                      fluff,fl_initial,p10,p20,p30,zo0,                      &
                      w_p1,w_p2,w_p3,                                        &
                      w_de,w_se,kc,i_min,r1max,r2max,r3max,alpha1,alpha2,         &
...
!BOC
   ...
   read(namlst,nml=bio_mab_nml,err=99)
   close(namlst)
...
#ifdef GOTM_BFM
!  Default saving behaviour: store all state variables
   pelvar_save_all = 1
#endif
...
!BOC
   ...
   if (fluff) then
      var_names(11) = 'flf'
      var_units(11) = 'mmol n/m**2'
      var_long(11)  = 'fluff'
   end if

   return
   end subroutine var_info_mab
!EOC
