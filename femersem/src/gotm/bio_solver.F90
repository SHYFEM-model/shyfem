...
! Allocate global variables
   if (.NOT. allocated(cc0)) then
      allocate(cc0(1:numc,0:nlev))
      allocate(cc1(1:numc,0:nlev))
      allocate(rhs(1:numc,0:nlev))
      allocate(rhs1(1:numc,0:nlev))
      allocate(rhs2(1:numc,0:nlev))
      allocate(rhs3(1:numc,0:nlev))
      allocate(ppsum(1:numc,0:nlev))
      allocate(ddsum(1:numc,0:nlev))
#ifdef BFM_GOTM
...
#ifdef BFM_GOTM
  REALTYPE :: ppbsum(1:numc,0:nlev),ddbsum(1:numc,0:nlev)
#endif
!EOP
!-----------------------------------------------------------------------
!BOC
   ...
   if (bio_setup>1) then
     do ci=1,NLEVB
        do i=1,numbc
           a(i,i)=0.
           do j=1,numbc
              a(i,i)=a(i,i)+ddb(i,j,ci)
              if (i.ne.j) a(i,j)=-dt*ppb(i,j,ci)/ccb(j,ci)
           end do
           a(i,i)=dt*a(i,i)/ccb(i,ci)
           a(i,i)=_ONE_+a(i,i)
           r(i)=ccb(i,ci)+dt*ppb(i,i,ci)
        end do
   ...
   if (bio_setup>1) then
     ...
     do ci=1,NLEVB
        do i=1,numbc
           ...
           do j=1,numbc
              a(i,i)=a(i,i)+ddb(i,j,ci)
              if (i.ne.j) a(i,j)=-dt*ppb(i,j,ci)/ccb0(j,ci)
           end do
           a(i,i)=dt*a(i,i)/ccb0(i,ci)
           a(i,i)=1.+a(i,i)
           r(i)=ccb0(i,ci)+dt*ppb(i,i,ci)
        end do
        call matrix(numbc,a,r,ccb(:,ci))
     end do
     ! reset diagonal terms only
