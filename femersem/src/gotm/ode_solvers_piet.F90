...
! !INTERFACE:
#ifdef BFM_GOTM
   subroutine ode_solver(bio_model,solver,numc,nlev,dt,h,cc,pp,dd,t)                !BFM
#else
...
!
!  Piet Ruardij: all changes are made to call the solver for different sets of
!  state variables and rates. IN this case for pelagic (cc,pp and dd) and benthic (ccb,ppb,ddb)
!
! !LOCAL VARIABLES:
!EOP
!-----------------------------------------------------------------------
!BOC
   select case (solver)
      case (1)
         call euler_forward(bio_model,dt,numc,nlev,cc,pp,dd,h,t)                    !BFM
      case (2)
         call runge_kutta_2(bio_model,dt,numc,nlev,cc,pp,dd,h,t)                    !BFM
      case (3)
         call runge_kutta_4(bio_model,dt,numc,nlev,cc,pp,dd,h,t)                    !BFM
      case (4)
         call patankar(bio_model,dt,numc,nlev,cc,pp,dd,h,t)                         !BFM
      case (5)
         call patankar_runge_kutta_2(bio_model,dt,numc,nlev,cc,pp,dd,h,t)           !BFM
      case (6)
         call patankar_runge_kutta_4(bio_model,dt,numc,nlev,cc,pp,dd,h,t)           !BFM
      case (7)
         call modified_patankar(bio_model,dt,numc,nlev,cc,pp,dd,h,t)                !BFM
      case (8)
         call modified_patankar_2(bio_model,dt,numc,nlev,cc,pp,dd,h,t)              !BFM
      case (9)
         call modified_patankar_4(bio_model,dt,numc,nlev,cc,pp,dd,h,t)              !BFM
      case (10)
         call emp_1(bio_model,dt,numc,nlev,cc,pp,dd,h,t)                            !BFM
      case (11)
         call emp_2(bio_model,dt,numc,nlev,cc,pp,dd,h,t)                            !BFM
      case default
...
!
! !INTERFACE:
   subroutine euler_forward(bio_model,dt,numc,nlev,cc,pp,dd,h,t)                    !BFM
!
! !DESCRIPTION:
...
!
! !INPUT PARAMETERS:
   integer, intent(in)                 :: bio_model                                 !BFM
   REALTYPE, intent(in)                :: dt
   integer, intent(in)                 :: numc,nlev
   REALTYPE, intent(in)                :: h(0:nlev)
   REALTYPE, intent(in)                :: t(0:nlev)
   REALTYPE, intent(inout)              :: pp(1:numc,1:numc,0:nlev)                  !BFM
   REALTYPE, intent(inout)              :: dd(1:numc,1:numc,0:nlev)                  !BFM
!
! !INPUT/OUTPUT PARAMETER:
  REALTYPE, intent(inout)              :: cc(1:numc,0:nlev)
!
...
!BOC
   ...
   call process_model(bio_model,first,numc,nlev,cc,pp,dd,h,t)                       !BFM
...
!
! !INTERFACE:
   subroutine runge_kutta_2(bio_model,dt,numc,nlev,cc,pp,dd,h,t)                    !BFM
!
! !DESCRIPTION:
...
!BOC
   ...
   call process_model(bio_model,first,numc,nlev,cc1,pp,dd,h,t)                      !BFM
...
!
! !INTERFACE:
   subroutine runge_kutta_4(bio_model,dt,numc,nlev,cc,pp,dd,h,t)                    !BFM
!
! !DESCRIPTION:
...
!
! !INTERFACE:
   subroutine patankar(bio_model,dt,numc,nlev,cc,pp,dd,h,t)                         !BFM
!
! !DESCRIPTION:
...
!BOC
   first=.true.
   call process_model(bio_model,first,numc,nlev,cc,pp,dd,h,t)                       !BFM
   cc = (cc+dt*sum(pp(:,:,:),2))/(1.0D+00+dt*sum(dd(:,:,:),2)/cc)
...
!
! !INTERFACE:
   subroutine patankar_runge_kutta_2(bio_model,dt,numc,nlev,cc,pp,dd,h,t)           !BFM
!
! !DESCRIPTION:
...
!
! !INTERFACE:
   subroutine patankar_runge_kutta_4(bio_model,dt,numc,nlev,cc,pp,dd,h,t)           !BFM
!
! !DESCRIPTION:
...
!
! !INTERFACE:
   subroutine modified_patankar(bio_model,dt,numc,nlev,cc,pp,dd,h,t)                !BFM
!
! !DESCRIPTION:
...
!
! !INTERFACE:
   subroutine modified_patankar_2(bio_model,dt,numc,nlev,cc,pp,dd,h,t)              !BFM
!
! !DESCRIPTION:
...
!BOC
   ...
   call process_model(bio_model,first,numc,nlev,cc1,pp1,dd1,h,t)                     !BFM
...
!
! !INTERFACE:
   subroutine modified_patankar_4(bio_model,dt,numc,nlev,cc,pp,dd,h,t)              !BFM
!
! !DESCRIPTION:
...
!BOC
   ...
   call process_model(bio_model,first,numc,nlev,cc1,pp2,dd2,h,t)                    !BFM
   ...
   call process_model(bio_model,first,numc,nlev,cc1,pp3,dd3,h,t)                    !BFM
...
!
! !INTERFACE:
   subroutine emp_1(bio_model,dt,numc,nlev,cc,pp,dd,h,t)                            !BFM
!
! !DESCRIPTION:
...
!
! !INTERFACE:
   subroutine emp_2(bio_model,dt,numc,nlev,cc,pp,dd,h,t)                            !BFM
!
! !DESCRIPTION:
...
!        J set of the EMP scheme.
         if (cc(i).eq.0.)STDERR "Error: state variable ",i," is zero and has negative derivative!"
...
!  Then EMP is not suitable (it will stall state variable values), so warn user.
   if (pi.lt.1.d-4) then
     STDERR "Warning: small pi=",pi," in Extended Modified Patankar slows down system!"
