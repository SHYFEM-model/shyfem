...
template <typename ValueType>
void HostMatrixCSR<ValueType>::CopyFromHostCSR(const int *row_offset, const int *col, const ValueType *val,
                                               const int nnz, const int nrow, const int ncol) {
...
// Y. Saad, Iterative methods for sparse linear systems, 2nd edition, SIAM
template <typename ValueType>
bool HostMatrixCSR<ValueType>::ILUTFactorize(const double t, const int maxrow) {
...
// this = A * B
template <typename ValueType>
bool HostMatrixCSR<ValueType>::SymbolicMatMatMult(const BaseMatrix<ValueType> &A, const BaseMatrix<ValueType> &B) {
...
template <typename ValueType>
bool HostMatrixCSR<ValueType>::NumericMatMatMult(const BaseMatrix<ValueType> &A, const BaseMatrix<ValueType> &B) {
...
template <typename ValueType>
bool HostMatrixCSR<ValueType>::SymbolicPower(const int p) {
...
template <typename ValueType>
bool HostMatrixCSR<ValueType>::ILUpFactorizeNumeric(const int p, const BaseMatrix<ValueType> &mat) {
...
template <typename ValueType>
bool HostMatrixCSR<ValueType>::Compress(const double drop_off) {
...
// ----------------------------------------------------------
template <typename ValueType>
bool HostMatrixCSR<ValueType>::AMGConnect(const ValueType eps, BaseVector<int> *connections) const {
...
// ----------------------------------------------------------
template <typename ValueType>
bool HostMatrixCSR<ValueType>::AMGAggregate(const BaseVector<int> &connections, BaseVector<int> *aggregates) const {
...
// ----------------------------------------------------------
template <typename ValueType>
bool HostMatrixCSR<ValueType>::AMGSmoothedAggregation(const ValueType relax,
    ...
    if (nnz_row == 1) {
      ...
      for (int k=0; k<nnz_row; ++k) {
        ...
        for (int aj=row_begin; aj<row_end; ++aj) {
          for (int j=0; j<nnz_row; ++j) {
            ...
            if (this->mat_.col[aj] == Asub_col)
              Asub[k+j*nnz_row] = this->mat_.val[aj];
      ...
      for (int i=0; i<nnz_row-1; ++i) {
        for (int k=i+1; k<nnz_row; ++k) {
          ...
          Asub[k+i*nnz_row] /= Asub[i+i*nnz_row];

          for (int j=i+1; j<nnz_row; ++j)
            Asub[k+j*nnz_row] -= Asub[k+i*nnz_row] * Asub[i+j*nnz_row];
...
//
template <typename ValueType>
bool HostMatrixCSR<ValueType>::Assemble(const int *jj, const int *ii, const ValueType *sr,
...
template <typename ValueType>
bool HostMatrixCSR<ValueType>::AssembleUpdate(const ValueType *sr,
