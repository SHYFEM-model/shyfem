...
bool paralution_get_platform_ocl(cl_platform_id **ocl_platforms, cl_uint *ocl_numPlatforms) {
  ...
  cl_int err = clGetPlatformIDs(0, NULL, ocl_numPlatforms);
...
bool paralution_get_device_ocl(const cl_platform_id &ocl_platform, cl_device_id **ocl_devices,
  ...
  cl_int err = clGetDeviceIDs(ocl_platform, CL_DEVICE_TYPE_ALL, 0, NULL, ocl_numDevices);
  if (err != CL_SUCCESS) {
...
bool paralution_set_kernels_ocl(cl_kernel *ocl_kernels) {
  ...
  for (int i=0; i<KERNELCOUNT; ++i) {
    ...
    ocl_kernels[k] = clCreateKernel(OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_program_sp, kernels_ocl[i].c_str(), &err);
    if (err != CL_SUCCESS) {
    ...
    ocl_kernels[k] = clCreateKernel(OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_program_dp, kernels_ocl[i].c_str(), &err);
    if (err != CL_SUCCESS) {
...
template <>
cl_kernel paralution_get_kernel_ocl<double>(const int kernel) {

  // return double kernel
  return (OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_kernels)[kernel/2 + 1];

}

// Returns the kernel for single precision
template <>
cl_kernel paralution_get_kernel_ocl<float>(const int kernel) {

  // return float kernel
  return (OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_kernels)[kernel/2];

}

template <>
cl_kernel paralution_get_kernel_ocl<int>(const int kernel) {

  LOG_INFO("This function is not supported for int on OpenCL backend");
...
bool paralution_init_ocl(void) {
  ...
  assert (_get_backend_descriptor()->OCL_handle == NULL);

  // OpenCL specific variables
  _get_backend_descriptor()->OCL_handle = new oclHandle_t;
  ...
  if (_get_backend_descriptor()->OCL_plat == -1 || _get_backend_descriptor()->OCL_dev == -1) {
    ...
    for (cl_uint p=0; p<ocl_numPlatforms; ++p) {
      for (cl_uint d=0; d<ocl_numDevices[p]; ++d) {
        ...
        if (ocl_typeDevice & CL_DEVICE_TYPE_GPU) {
          GPU = true;
          _get_backend_descriptor()->OCL_plat = p;
          _get_backend_descriptor()->OCL_dev  = d;
          p = ocl_numPlatforms;
          break;
        }
    ...
    if (GPU == false) {
      _get_backend_descriptor()->OCL_plat = 0;
      _get_backend_descriptor()->OCL_dev  = 0;
    }

  }

  // Check for valid platform
  if ((cl_uint)(_get_backend_descriptor()->OCL_plat) >= ocl_numPlatforms ||
      _get_backend_descriptor()->OCL_plat < 0) {
  ...
  if ((cl_uint)(_get_backend_descriptor()->OCL_dev) >= ocl_numDevices[_get_backend_descriptor()->OCL_plat] ||
      _get_backend_descriptor()->OCL_dev < 0) {
  ...
  err = clGetDeviceInfo(ocl_devices[_get_backend_descriptor()->OCL_plat][_get_backend_descriptor()->OCL_dev],
                        CL_DEVICE_MAX_WORK_GROUP_SIZE,
                        sizeof(_get_backend_descriptor()->OCL_max_work_group_size),
                        &_get_backend_descriptor()->OCL_max_work_group_size,
                        NULL);
  if (err != CL_SUCCESS) {
  ...
  if (ocl_typeDevice & CL_DEVICE_TYPE_CPU) _get_backend_descriptor()->OCL_max_work_group_size /= 16;
  if (ocl_typeDevice & CL_DEVICE_TYPE_GPU) _get_backend_descriptor()->OCL_max_work_group_size = 256;

  err = clGetDeviceInfo(ocl_devices[_get_backend_descriptor()->OCL_plat][_get_backend_descriptor()->OCL_dev],
                        CL_DEVICE_MAX_COMPUTE_UNITS,
                        sizeof(_get_backend_descriptor()->OCL_computeUnits),
                        &_get_backend_descriptor()->OCL_computeUnits,
                        NULL);
  if (err != CL_SUCCESS) {
    LOG_INFO("Cannot query for CL_DEVICE_TYPE");
    paralution_stop_ocl();
  ...
  if (_get_backend_descriptor()->OCL_warp_size == -1)
    _get_backend_descriptor()->OCL_warp_size = 32;

  // Create OpenCL context for computing device
  OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_context = clCreateContext(0, 1,
                               &ocl_devices[_get_backend_descriptor()->OCL_plat][_get_backend_descriptor()->OCL_dev],
                               NULL, NULL, &err);
  if (err != CL_SUCCESS) {
  ...
  OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_cmdQueue = clCreateCommandQueueWithProperties(
                                 OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_context,
                                 ocl_devices[_get_backend_descriptor()->OCL_plat][_get_backend_descriptor()->OCL_dev],
                                 NULL, &err);
#else
  OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_cmdQueue = clCreateCommandQueue(
                                 OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_context,
                                 ocl_devices[_get_backend_descriptor()->OCL_plat][_get_backend_descriptor()->OCL_dev],
                                 0, &err);
#endif
  ...
  std::stringstream ss;
  ss << "#define BLOCK_SIZE " << _get_backend_descriptor()->OCL_max_work_group_size << "\n";
  ss << "#define WARP_SIZE "  << _get_backend_descriptor()->OCL_warp_size  << "\n";
  sourceStr_sp  = ("#pragma OPENCL EXTENSION cl_khr_fp64 : enable\n");
  sourceStr_sp.append("#define IndexType int\n");
  ...
  sourceStr_dp.append("#define IndexType int\n");
  ...
  const char *source_sp[] = { sourceStr_sp.c_str() };
  const char *source_dp[] = { sourceStr_dp.c_str() };

  // Create OpenCL program for single precision kernels
  OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_program_sp = clCreateProgramWithSource(OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_context,
                                                                                                1, source_sp, NULL, &err);
  if (err != CL_SUCCESS) {
  ...
  OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_program_dp = clCreateProgramWithSource(OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_context,
                                                                                                1, source_dp, NULL, &err);
  if (err != CL_SUCCESS) {
  ...
  err  = clBuildProgram(OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_program_sp,
                        0, NULL, "-w -cl-mad-enable -cl-no-signed-zeros", NULL, NULL);
  err |= clBuildProgram(OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_program_dp,
                        0, NULL, "-w -cl-mad-enable -cl-no-signed-zeros", NULL, NULL);
  if (err == CL_BUILD_PROGRAM_FAILURE) {

    // If compilation failed, get and print build log (this happens only for 1 program)
    LOG_INFO("CL_BUILD_PROGRAM_FAILURE");

    size_t logsize;
    err = clGetProgramBuildInfo(OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_program_sp,
                                ocl_devices[_get_backend_descriptor()->OCL_plat][_get_backend_descriptor()->OCL_dev],
    ...
    if (err != CL_SUCCESS) {
      LOG_INFO("Cannot get compiler error log");
      paralution_stop_ocl();
      return false;
    }
  ...
  // Allocate kernel array for double + float kernels
  OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_kernels = new cl_kernel[2*KERNELCOUNT];
  // Write kernels to backend kernel array
  stat = paralution_set_kernels_ocl(OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_kernels);
  if (stat != true)
    return false;

  OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_platforms = ocl_platforms;
  OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_numPlatforms = ocl_numPlatforms;
  OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_devices = ocl_devices;
  OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_numDevices = ocl_numDevices;
...
void paralution_stop_ocl(void) {
  ...
  if (_get_backend_descriptor()->accelerator) {
    ...
    if (OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_cmdQueue) {
      err = clReleaseCommandQueue(OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_cmdQueue);
      CHECK_OCL_ERROR(err, __FILE__, __LINE__);
      OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_cmdQueue = NULL;
    }

    // If OpenCL kernel was created, release it
    if (OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_kernels != NULL) {
      for (int i=0; i<2*KERNELCOUNT; ++i) {
        if (OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_kernels[i] != NULL) {
          err = clReleaseKernel(OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_kernels[i]);
          CHECK_OCL_ERROR(err, __FILE__, __LINE__);
        }
      }
      delete[] OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_kernels;
    }

    // If OpenCL program was created, release it
    if (OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_program_sp) {
      err = clReleaseProgram(OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_program_sp);
      CHECK_OCL_ERROR(err, __FILE__, __LINE__);
      OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_program_sp = NULL;
    }
    // If OpenCL program was created, release it
    if (OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_program_dp) {
      err = clReleaseProgram(OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_program_dp);
      CHECK_OCL_ERROR(err, __FILE__, __LINE__);
      OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_program_dp = NULL;
    }

    // If OpenCL context was created, release it
    if (OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_context) {
      err = clReleaseContext(OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_context);
      CHECK_OCL_ERROR(err, __FILE__, __LINE__);
      OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_context = NULL;
    }

    // If OpenCL platforms were queried, release them
    if (OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_platforms != NULL)
      delete[] OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_platforms;

    // If OpenCL devices were queried, release them
    if (OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_devices != NULL) {
      for (cl_uint p=0; p<OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_numPlatforms; ++p)
        if (OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_devices[p] != NULL)
          delete[] OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_devices[p];
      delete[] OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_devices;
    }

    if (OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_numDevices != NULL)
      delete[] OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_numDevices;

    OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_numPlatforms = 0;

  }

  delete static_cast<oclHandle_t*> (_get_backend_descriptor()->OCL_handle);
  _get_backend_descriptor()->OCL_handle = NULL;
...
void paralution_info_ocl(const struct Paralution_Backend_Descriptor backend_descriptor) {
  ...
  if (_get_backend_descriptor()->OCL_dev >= 0) {
  ...
  for (cl_uint p=0; p<OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_numPlatforms; ++p) {
    ...
    for (cl_uint d=0; d<OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_numDevices[p]; ++d) {

      err = clGetPlatformInfo(OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_platforms[p],
                              CL_PLATFORM_NAME, sizeof(ocl_namePlatform), &ocl_namePlatform, NULL);
      CHECK_OCL_ERROR(err, __FILE__, __LINE__);

      // Get and print OpenCL device name
      err = clGetDeviceInfo(OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_devices[p][d],
      ...
      err = clGetDeviceInfo((OCL_HANDLE(_get_backend_descriptor()->OCL_handle)->OCL_devices)[_get_backend_descriptor()->OCL_plat][_get_backend_descriptor()->OCL_dev],
                            CL_DEVICE_TYPE,
                            sizeof(ocl_typeDevice),
                            &ocl_typeDevice,
                            NULL);
