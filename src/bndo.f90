...
!==================================================================
	...
	double precision, allocatable, save :: xynorm(:,:)		!normal direction
	double precision, allocatable, save :: wopnodes(:,:)	!weights

!==================================================================
	contains
!==================================================================

	subroutine mod_bndo_init(ngr,nrb,nrbex)

	integer ngr,nrb

	integer nlk,naux,naex
	integer, optional :: nrbex
	...
	if(present(nrbex)) then
          allocate(ibcnod(naex))
          allocate(kbcnod(naex))
          allocate(itynod(naex))
          allocate(nopnod(naex))
          allocate(nopnodes(ngr,naex))
          allocate(xynorm(2,naex))
          allocate(wopnodes(ngr,naex))
	else
