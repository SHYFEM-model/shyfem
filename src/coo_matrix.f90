!
! $Id: subcoo.f,v 1.3 2009-03-31 15:37:07 georg Exp $
!
...
! 15.12.2015    ggu&deb finished and validated 3d approach
!
!******************************************************************
!----------------------------------------------------------------------
        module coo_matrix
...
!------------------------------------------------------------------
	...
	do k=1,nkn_local
          if(bmpi) then
	    call pget_nodes_around(k,maxlnk,n,nodes)
          else
...
!------------------------------------------------------------------
...
#ifdef DEBUGON
	n2zero = ntg(nkn_local)	!set global 2d value
#else
	...
	do ie=1,nel
	  ...
	  do i=1,3
	    ki = kn(i)				!row
            if (bmpi) then
              ki_glob = domain%nodes%globalID(ki)
            else
              ki_glob = ki
            end if
	    ...
	    do j=1,3
	      kj = kn(j)			!col
              if (bmpi) then
                kj_glob = domain%nodes%globalID(kj)
              else
                kj_glob = kj
              end if
	      do m=1,n
...
#ifdef DEBUGON
              if((ki.le.nkn_inner)) ijp_ie(i,j,ie) = ipp
#else
	      ijp_ie(i,j,ie) = ipp
#endif
	      i2coo(ipp) = ki
...
! checks sanity of iijp, ip, jp arrays
	...
	if( debug ) then
	  ...
	  write(6,*) '  double precision band   = ',nnkn + mmbw * (2*nnkn-mmbw-1)
...
! co,ico,jco,iijp,nnz
      ...
      integer nnkn,mmbw
      integer nnz
      double precision co(ndim),coon(ndim)
...
!         nonzero elements of the matrix with a(k) = actual double precision value of
...
! 	continung the double precision values, jao containing the column indices, 
...
! values= logical indicating whether or not the double precision values a(*) must 
...
!-----------------------------------------------------------------------
! this subroutine performs an in-place permutation of a double precision vector x 
...
!
! abd   = double precision array containing the values of the matrix stored in
