...
! subroutine makehkv(hkv,haux)		makes hkv (nodewise depth)
...
#ifdef DEBUGON
        integer e
        double precision hkv(nkn_local)
        double precision haux(nkn_local)   !aux array -> bug - was integer
#else
        double precision hkv(nkn)
