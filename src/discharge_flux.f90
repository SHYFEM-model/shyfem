!
! $Id: subflx.f,v 1.25 2009-05-21 09:24:00 georg Exp $
!
...
! passed in are pointers to these section in lnk structure
	...
	double precision az			!time weighting parameter
	double precision flux		!flux computed (return)
	...
	double precision transp(maxlnk)
...
! -> has been done - other sources of mfluxv (rain, etc.) are also eliminated
	...
	integer n		!dimension/size of transp (entry/return)
	double precision transp(nlvdi,n)	!computed fluxes (return)
	integer ne		!total number of elements in elems
	integer elems(ne)	!elements around k
...
! passed in are pointers to these section in lnk structure
	...
	double precision az			!time weighting parameter
	integer lkmax		!maximum layer in finite volume k (return)
	double precision flux(nlvdi)	!computed fluxes (return)
	...
	integer elems(maxlnk)
	double precision transp(nlvdi,ngr)
...
! else uses kantv
	...
	if( kantv(1,k) .eq. 0 ) then			!inner point
	   ...
	   kafter = kantv(1,k)
	   kbefor = kantv(2,k)
	   if( is_external_boundary(kbefor) .and. is_external_boundary(kafter) ) then	!OOO
		ktype = 5
...
! if on boundary (itype>1) rflux(n) is not used (because not defined)
	...
	double precision rflux(nlvdi,n)	!fluxes into node (element)
	double precision tflux(nlvdi,n)	!fluxes through sides (return value)

	integer i,l
	double precision rf(ngr)
	double precision tf(ngr)
...
! if on boundary (itype>1) rflux(n) is not used (because not defined)
	...
	integer itype		!type of node (1=int,2=bnd,3=BOO,4=OOB,5=OOO)
	integer n		!number of sides (tfluxes)
	double precision rflux(n)	!fluxes into node (element)
	double precision tflux(n)	!fluxes through sides (return value)
