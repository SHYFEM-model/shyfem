...
!-----------------------------------------------------------------------
	...
	subroutine heatcoare(airt,airp,ws,rh,cloud,sst,prec,rad,qsens,qlat,qlong,evap,cd)
...
! input
        double precision, intent(in)    :: airt       !air temperature [C] 
        double precision, intent(in)    :: airp       !pressure [mb]               
        double precision, intent(in)    :: ws         !wind speed [m/s] 
        double precision, intent(in)    :: rh         !relative humidity [%] ([0-100])
        double precision, intent(in)    :: cloud      !cloud cover [0-1] 
        double precision, intent(in)    :: sst        !sea temperature [C]                 
        double precision, intent(in)    :: prec       !precipitation [m/s]
	double precision, intent(in)    :: rad	  !net solar flux (w/m^2)
! inout
! output
        double precision,intent(out)    :: qsens      !sensible heat flux [W/m**2]
        double precision,intent(out)    :: qlat       !latent heat flux [W/m**2] 
        double precision,intent(out)    :: qlong      !long wave radiation [W/m**2] 
        double precision,intent(out)    :: evap       !evaporation [kg/m**2/s] 
	double precision,intent(out)    :: cd	  !drag_coefficient
! local
	integer, parameter  :: lon_method=2
	double precision 		    :: ta,tw,rain
	double precision 		    :: es,ea,qs,qa,rhoa
...
! input
	double precision, intent(in)        :: hum		!humidity [%]
	double precision, intent(in)        :: airp		!air pressure [mb]
	double precision, intent(in)        :: tw		!water temp [C]
	double precision, intent(in)        :: ta		!air temp [C]
! output
	double precision, intent(out)       :: qs		!bulk water spec hum (kg/kg)
	double precision, intent(out)	:: qa		!bulk air spec hum (kg/kg)
	double precision, intent(out)       :: rhoa		!moist air density (kg/m3)
	double precision, intent(out)       :: es
	double precision, intent(out)       :: ea
! local
	double precision        		:: rh,pres
...
! input
	double precision, intent(in)	:: t		!temperature [C]
	double precision, intent(in)	:: p		!air pressure [mb]
! output
	double precision, intent(out)	:: esat 	!saturation vp [mb]
...
! parameters
	...
	double precision, parameter     :: sqr3 = 1.7320508
! local
	double precision                :: Fw,chic,chik,psic,psik,c
...
! input
	...
	double precision, intent(in)                :: tw_c,ta_c,ea,qa,cloud
! output
	double precision, intent(out)               :: qb
! parameters
	...
	double precision, parameter, dimension(91)  :: cloud_correction_factor = (/ & 
...
! input
	double precision, intent(in)      :: sst		!water temperature [C]
	double precision, intent(in)      :: airt		!air temperature [C]
	double precision, intent(in)      :: w		!wind speed [m/s]
	double precision, intent(in)      :: rain  		!precipitation [kg/m2/s] 
        double precision, intent(in)      :: qs		!bulk water spec hum [kg/kg]
        double precision, intent(in)      :: qa		!bulk air spec hum [kg/kg]
        double precision, intent(in)      :: rhoa		!moist air density [kg/m3]
! output
	double precision, intent(out)     :: evap		!evaporation [kg/(m**2 s)]
	double precision, intent(out)     :: qe		!sensible heat flux [W/m**2]
	double precision, intent(out)     :: qh		!latent heat flux [W/m**2]
	double precision, intent(out)     :: Cd		!wind dragg coeff.

! parameters
!  Fairall LKB roughness Reynolds number to Von Karman
	double precision, parameter       :: fdg = 1.0          ! non-dimensional

!  Beta parameter evaluated from Fairall low windspeed turbulence data.
	double precision, parameter       :: beta = 1.2         ! non-dimensional

!  Zabl      Height (m) of atmospheric boundary layer.
	double precision, parameter       :: Zabl = 600.0       ! in [m]
	double precision, parameter       :: r3 = 1.0/3.0
!
...
!
	double precision,parameter, dimension(8,2) :: Liu_a = reshape (     & 
	...
	double precision,parameter, dimension(8,2) :: Liu_b = reshape (     &
	...
	double precision,parameter, dimension(9) :: Liu_Rr =        & 
     &           (/0.0, 0.11, 0.825, 3.0, 10.0, 30.0, 100.0, 300.0, 1000.0 /)

        double precision, parameter	      :: charn= 0.011        !Charnock
	double precision, parameter       :: wgust= 0.2
	double precision, parameter       :: zt = 2.0  !Height (m) of air temp. measurement.
	double precision, parameter       :: zq = 2.0  !Height (m) of air hum. measurement
	double precision, parameter       :: zw = 10.0 !Height (m) of winds measurement
	integer,  parameter   :: itermax = 20
! local
	double precision        :: cff,wgus
	double precision        :: L
	double precision        :: ta,ta_k,tw
	integer     :: iter,k
	double precision        :: vis_air
	double precision        :: tpsi,qpsi,wpsi,ZWoL,oL,ZToL,ZQoL,ZoW,ZoT, ZoQ
	double precision        :: Wstar,Tstar, Qstar, delQ, delT, rr,rt,rq
	double precision        :: TVstar,Bf, upvel,delw,Wspeed
	double precision        :: cd_rain
	double precision        :: x1,x2,x3
...
! input
        double precision, intent(in) 	:: airt         !air temperature [C]
        double precision, intent(in) 	:: sst          !sea surface temperature [C]
        double precision, intent(in) 	:: ws           !wind speed [m/s]                    
        double precision, intent(in) 	:: rain         !precipitation [kg/m2/s]
	double precision, intent(in) 	:: Rns	        !net solar flux (W/m^2)
	double precision, intent(in) 	:: Rnl	        !net longwave flux (W/m^2)
	double precision, intent(in) 	:: Qs,Q	        !Spec. humidity	[kg/kg]		   
	double precision, intent(in) 	:: rhoa	        !moist air density (kg/m3)	   
! output
        double precision, intent(out) 	:: qsens        !sensible heat flux [W/m**2]         
        double precision, intent(out) 	:: qlat		!latent heat flux [W/m**2]           
        double precision, intent(out) 	:: evap		!evaporation [kg/m**2/s]             
	double precision, intent(out)	:: Cd		!dragg coefficient		   
! parameters
	double precision, parameter		:: Beta = 1.2 
	double precision, parameter		:: fdg  = 1.00 
	double precision, parameter		:: be   = 0.026 
	double precision, parameter		:: tcw  = 0.6 
	double precision, parameter		:: zu   = 10.  !wind speed measurement height (m)
	double precision, parameter		:: zt   = 2.   !air T measurement height (m)
	double precision, parameter		:: zq   = 2.   !air q measurement height (m)
	double precision, parameter		:: zi   = 600. !PBL depth (m)
! local
	double precision 	:: us		!surface current speed in the wind direction (m/s)
	integer	:: jcool	!implement cool calculation skin switch, 0=no, 1=yes
	integer	:: jwave	!implement wave dependent roughness model
	double precision 	:: twave, hwave	!wave period (s) and height (m)
	integer	:: i,nits	!iteration
	double precision 	:: Le,visa	!air constants
	double precision 	:: Al,bigc,wetc	!cool skin constants
	double precision 	:: lwave,cwave	!wave parameters
	double precision 	:: du,dt,dq,ta,ug,dter,dqer,ut
	double precision 	:: u10,usr,zo10,Cd10,Ch10,Ct10,zot,Ct,CC
	double precision 	:: Ribcu,Ribu,zetu,L10
	double precision 	:: tsr,qsr,tkt,charn
	double precision 	:: zet,L,zoq,Bf
	double precision 	:: zot10,zo,rr
	double precision 	:: hsb,hlb,qout,dels,qcol,alq,xlamx
	double precision 	:: tau,dwat,dtmp,alfac,RF
	double precision 	:: Ch,Ce
...
!	------------------------------------------------
	...
	visa = 1.326e-5*(1+6.542e-3*airt+8.301e-6*airt*airt - 4.84e-9*airt*airt*airt) 
...
!	------------------------------------------------
	...
	dtmp=(1.+3.309e-3*airt-1.44e-6*airt*airt)*0.02411/(rhoa*cpa) !heatdiffusivity
	alfac= 1./(1.+(wetc*Le*dwat)/(cpa*dtmp))    	!wet bulb factor
	RF= rain*alfac*cpw*((sst-airt-dter*jcool)+(Qs-Q-dqer*jcool)*Le/cpa)
...
! input
	double precision, intent(in)	:: ws		!wind speed [m/s]
	double precision, intent(in)	:: rhoa		!moist air density (kg/m3)
	double precision, intent(in)	:: rhow		!water density (kg/m3)
	double precision, intent(in)	:: rain		!precipitation (kg/m2/s)
        double precision, intent(in)	:: wx,wy      	!wind components [m/s]
! output
	double precision, intent(out)	:: taux,tauy	!wind stress components
! local
...
! input
	double precision, intent(in)	:: rad	!Net shortwave flux [W/m2] (+ down)
	double precision, intent(in)	:: qrad	!LH + SH + LW [W/m2] (+ down)
	double precision, intent(in)	:: tw 	!sea temperature at depth hb [C]
	double precision, intent(in)	:: hb	!depth of modelled tw [m]
	double precision, intent(in)	:: usw	!surface friction velocity [m/s]
	double precision, intent(in)	:: dt 	!time step [s]
! input-output
	double precision, intent(inout)	:: dtw	!Warm layer temp. diff. [C]
! output
	double precision, intent(out)	:: tws	!skin temperature [C]
! parameters
	double precision, parameter		:: num  = 0.3    !exponent of warm layer T profile
! local
	double precision			:: sw	!Net shortwave flux (degC m/s), + down
	double precision			:: q	!LH + SH + LW [degC m/s] (+ down)
	double precision			:: alw	!thermal expansion coefficient
	double precision			:: nu	!num computed following Lee et al, 2013
	double precision 			:: zeta	!db / L  (L is Monin-Obukhov length)
	double precision 			:: qn	!Q + R_s - R(-d) heat flux in warm layer
	double precision			:: ds	!cool skin layer depth (m; positive)
	double precision			:: fs	!frac. of solar rad. absorbed in the cool sublayer
	double precision			:: f1	!frac. of solar rad. absorbed in the warm layer
	double precision			:: phi	!stability function
	double precision			:: dtc	!Cool skin temp. diff. (deg C)
	double precision			:: dtwo	!dtw frmo previous time step
	double precision			:: q2,qn1
	double precision			:: cff,cff1,cff2,cff3,cff4,cff5
...
!	------------------------------------------------
        ...
        dtw = (dtwo + (nu+1.)/nu*(q+sw*f1)*dt/hb)/(1.+(nu+1.)*cff3*dt)
        dtw = max(0.,dtw)
