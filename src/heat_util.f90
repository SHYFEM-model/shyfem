!
! $Id: subqfxu1.f,v 1.7 2009-03-11 16:25:59 georg Exp $
!
...
! converts quantities dealing with vapor content in air
!
! dry air temperature (dp) must always be given
!
! dp <= wb <= db   ( rh == 100 % => dp == wb == db )
!
! mode indicates what value is given (a part from dry air temperature)
! 	1	relative humidity
!	2	wet bulb temperature
!	3	dew point temperature
!
! the other two values are computed as output

	implicit none

	integer mode	!what values is given:  1:rh  2:wb  3:dp
	double precision db		!(dry) air temperature [C]
	double precision rh		!relative humidity [%] (0-100)
	double precision wb		!wet bulb temperature [C]
	double precision dp		!dew point temperature [C]
...
! computes wet bulb temperature from temperature and relative humidity
	...
	double precision db		!(dry) air temperature [C]
	double precision rh		!relative humidity [%] (0-100)
	double precision wb		!wet bulb temperature [C] (output)

	integer isel	!select the mode
        double precision dp         !dew point temperature (not used)
...
! computes relative humidity from temperature and wet bulb temperature
	...
	double precision db		!(dry) air temperature [C]
	double precision wb		!wet bulb temperature [C]
	double precision rh		!relative humidity [%] (0-100) (output)

	integer isel	!select the mode
	double precision dp		!dew point
...
!     RH Umidita' relativa % [0-100]
      ...
      double precision DB,RH,WB,DP,PB,PV,W,V
...
! converts specific from/to relative humidity

	implicit none

	integer mode	! 0: relative to specific  1: specific to relative
	double precision rh		! relative humidity [%]
	double precision w		! specific humidity
	double precision tair	! air temperature [Celsius]

	double precision DB,WB,DP,PB,PV,H,V
...
!
!    w = specific humidity of air vapor mixture (kg/kg)
!    rh = relative humidity (%)
!    rhows = density of water vapor (kg/m3)
!    rho = density of the moist or humid air (kg/m3)
!
!	end
!
!*****************************************************************************
...
! example: PSDBWB uses DB and WB to compute DP,PV,W,H,V,RH
      ...
      SUBROUTINE PSDBWB(DB,WB,DP,PB,PV,W,H,V,RH)
        ...
        double precision DB,WB,DP,RH,H,PB,PV,W,V
      PVP=PVSF(WB)
      IF (DB-WB)1,4,7
    1 WRITE(*,10)DB,WB
   ...
   11 FORMAT(5X,'ERROR IN PSDBWB : DB=',F6.2,'  WB=',F6.2,'   temperatures not compatible')
    ...
    3 W=0.62198*PV/(PB-PV)
      ...
      SUBROUTINE PSDBDP(DB,WB,DP,PB,PV,W,H,V,RH)
        ...
        double precision DB,WB,DP,H,RH,PB,PV,W,V
      IF(DP .LE. DB) GO TO 1
      WRITE(*,10)DB,DP
   10 FORMAT(5X,'ERROR IN PSDBDP : DB=',F6.2,'  DP=',F6.2,'   DP greater than DB')
    ...
    1 PV=PVSF(DP)
      ...
      SUBROUTINE PSDBRH(DB,WB,DP,PB,PV,W,H,V,RH)
        ...
        double precision DB,WB,DP,PB,PV,W,H,V,RH
...
! computes dew point and wet bulb temperature from dry temp and rel. hum.
        ...
        double precision t          !dry air temperature
        double precision rh         !relative humidity
        double precision td         !dew point air temperature (out)
        double precision tw         !wet bulb temperature (out)

        double precision, parameter :: es0 = 0.611    !saturation vapor pressure in kPa
        double precision, parameter :: t0 = 237.3     !reference temperature ?

        double precision p,taux,es,e,lne,gamma,delta
...
!                    D = Dewpoint in Centigrade (C) degrees
	...
	double precision es,ew,e,b,d,w,p
...
! compute rel. hum and wet bulb from dew point and dry temp
	...
	implicit none
        ...
        double precision p,taux,es,e,lne,gamma,delta,ed
...
!***********************************************************************
	...
	integer i
	double precision db,rh,wb,dp,td,wb2,wb3
...
! -14.70000       76.00000
        ...
        double precision db,rh,wb,td,tw,dp
...
!***********************************************************************
	...
	subroutine test_specific
          ...
          double precision rh,rh1,w,w1,tair
