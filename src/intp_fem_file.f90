...
!================================================================
	...
	type  :: info
	  ...
	  integer :: nmpi = 0		!expected horizontal points mpi
	  ...
	  double precision :: flag = -999.		!flag for value not available
	  logical :: bneedall = .true.	!need values for all points

	  integer :: datetime(2) = 0	!date and time parameters
	  double precision :: regpar(7) = 0.	!parameters for regular grid
	  ...
	  double precision, allocatable :: data_file(:,:,:)
	  double precision, allocatable :: hlv_file(:)
	  integer, allocatable :: ilhkv_file(:)
	  double precision, allocatable :: hd_file(:)
	end type info
	...
	integer, parameter, private :: ndim = 500
	...
	double precision, save, allocatable :: hk_fem(:)
	double precision, save, allocatable :: he_fem(:)
	double precision, save, allocatable :: hlv_fem(:)
...
!================================================================
	...
	do id=ids,ide
	  ...
	  descrp = p%descript
	  write(iu,1000) id,pinfo(id)%ibc,pinfo(id)%iunit,pinfo(id)%nvar        &
     &			,pinfo(id)%nintp,pinfo(id)%iformat,descrp(1:10),name
	end do
...
!****************************************************************
	...
	call iff_delete_entry(id)
        if(pinfo(id)%iunit.gt.0) close(pinfo(id)%iunit)
	pinfo(id)%iunit = -3
...
! nvar is return value (if file can be read)
	...
	!---------------------------------------------------------
        ...
        if( .not. breg .and. nexp > 0 .and. nexp /= nkn_fem .and. nexp /= nel_fem) then
...
!****************************************************************
	...
	subroutine iff_init_mpi(dtime,file,nvar,nexp,nmpi,lexp,nintp,nodes,vconst,id,idb)
...
! nvar is return value (if file can be read)
	...
	idlast = idlast + 1
	if( idlast > ndim ) then
	  stop 'error stop iff_init_mpi: too many files opened'
	end if
	id = idlast
	...
	if( p%nexp /= nexp ) then	!dummy test - delete later
	  stop 'error stop iff_init_mpi: internal error (99)'
	end if
	...
	if( .not. breg .and. nmpi > 0 .and. nmpi /= nkn_fem .and. nmpi /= nel_fem) then
	  allocate(pinfo(id)%nodes(nmpi))	!lateral BC
          do i=1,nmpi
	    pinfo(id)%nodes(i) = nodes(i)
          end do
	end if
	...
	if( id0 > 0 ) then
	  ...
	  stop 'error stop iff_init_mpi: internal error (3)'
	end if
	...
	call iff_populate_records_mpi(id,dtime)
...
! (re-) sets constant if no file has been opened 
	...
	integer id
	double precision vconst(pinfo(id)%nvar)

	integer iformat,nexp
	logical bnofile
...
!****************************************************************
	...
	!double precision dtimes,ddt
		...
		!	dtimeold = dtime
                ...
                do
                  if( dtime0 == -1. ) exit	! no double precision time given
                  bok = iff_peek_next_record(id,dtime2)
                  if( .not. bok ) goto 97
...
!****************************************************************
	...
	subroutine iff_populate_records_mpi(id,dtime0)
	...
	logical bok,bts
        ...
        if( .not. iff_read_next_record_mpi(id,dtime) ) goto 99
	dtimefirst = dtime
	...
	if( bok ) then				!at least two records
		nintp = pinfo(id)%nintp
		call iff_assert(nintp > 0,'nintp<=0')
	        do   ! only when need to read the new time series
		  if( dtime0 == -1. ) exit	! no double precision time given
		  bok = iff_peek_next_record(id,dtime2)
                  if( .not. bok ) goto 97
		  if( dtime2 >= dtime0 ) exit
                  bok = iff_read_next_record_mpi(id,dtime)
                  if( .not. bok ) goto 97
		  dtimelast = dtime
		end do
   ...
   91	continue
	...
	write(6,*) 'looking for it = ',dtime0
	write(6,*) 'first time found = ',dtimefirst
	stop 'error stop iff_populate_records_mpi: no time record found'
   96	continue
	call iff_print_file_info(id)
	write(6,*) 'cannot find enough time records'
	write(6,*) 'would need at least ',nintp
	stop 'error stop iff_populate_records_mpi: not enough records'
   97	continue
	call iff_print_file_info(id)
	write(6,*) 'cannot find time record'
	!write(6,*) 'looking at least for it = ',dtimes
   ...
   98	continue
	...
	write(6,*) 'time step less than 0'
	write(6,*) 'this happens at it = ',dtime
	stop 'error stop iff_populate_records_mpi: time step <= 0'
   99	continue
	call iff_print_file_info(id)
	write(6,*) 'error reading first record of file'
	stop 'error stop iff_populate_records_mpi: read error'
	end  subroutine iff_populate_records_mpi
...
!****************************************************************
	...
	integer nexp,lexp,nvar,nintp,nmpi
	logical bonepoint
	...
	if( bonepoint ) then	! time series or constant - store only one point
	  ...
	  allocate(pinfo(id)%time(nintp))
          if((bmpi).and.(nexp /= nkn_fem).and.(nexp /= nel_fem)) then
	    allocate(pinfo(id)%data(lexp,nmpi,nvar,nintp))
          else
...
!****************************************************************
        ...
        function iff_read_next_record_mpi(id,dtime)

        use timing
        use shympi

	logical iff_read_next_record_mpi
	integer id
	double precision dtime,time1
	  ...
	  if(pinfo(id)%ireg .eq.1 ) then
	    call read_regular_grid_mpi(id) 
	  else
            call iff_read_data_mpi(id,dtime)
	  end if

          if(ln_timing) io_time = io_time + shympi_wtime() - time1

	  iff_read_next_record_mpi = .true.
	else
	  call iff_close_file(id)
	  iff_read_next_record_mpi = .false.
	end if

        end function iff_read_next_record_mpi
	...
	if( bnofile ) then
	  ...
	  if( ierr == 0 ) then
	    ...
	    call iff_adjust_datetime(id,pinfo(id)%datetime,dtime)
          ...
          call fem_file_peek_params(iformat,iunit,dtime,nvers,np,lmax,nvar,ntype,datetime,ierr)
	  if( ierr == 0 ) then
	    pinfo(id)%datetime = datetime
	    call iff_adjust_datetime(id,pinfo(id)%datetime,dtime)
	  end if
...
!****************************************************************
	...
	integer ivar,nexp
	double precision f(pinfo(id)%nvar)
	...
	if( bts ) then
	  ...
	  if( ierr == 0 ) then
	    ...
	    call iff_adjust_datetime(id,pinfo(id)%datetime,dtime)
          ...
          call fem_file_read_params(iformat,iunit,dtime,nvers,np,lmax,nvar,ntype,datetime,ierr)
	  if( ierr == 0 ) then
	    pinfo(id)%datetime = datetime
	    call iff_adjust_datetime(id,pinfo(id)%datetime,dtime)
	  end if
	...
	if( bts ) then
	  ...
	  pinfo(id)%ntype = ntype
	  call fem_file_read_2header(iformat,iunit,ntype,pinfo(id)%regpar,ierr)
        if (pinfo(id)%ireg .eq. 1) breg=.true.
	  if(bmpi .and. (icall.eq.0) .and. breg) then
	     call regular_grid_min_max_index(id,pinfo(id)%nmpi)
             icall = 1
          else if (bmpi .and. breg) then
            pinfo(id)%nmpi= (imaxreg-iminreg+1)* (jmaxreg-jminreg+1)
	  end if
	  call iff_allocate_file_arrays(id,nvar,np,lmax)

	  call fem_file_read_3header(iformat,iunit,lmax,pinfo(id)%hlv_file,ierr)
...
! if np/lmax == 0: only deallocate
	...
	if( np > 0 .or. lmax > 0 ) then
	  allocate(pinfo(id)%hlv_file(lmax))
          if(bmpi) then
	    allocate(pinfo(id)%data_file(lmax,nmpi,nvar))
	    allocate(pinfo(id)%ilhkv_file(nmpi))
	    allocate(pinfo(id)%hd_file(nmpi))
          else
...
!****************************************************************
	...
	if( bts ) then
	  ...
	  do i=1,nvar
            call fem_file_read_data(iformat,iunit,nvers,np,lmax,string,pinfo(id)%ilhkv_file     &
...
!****************************************************************
        ...
        subroutine iff_read_data_mpi(id,dtime)
	...
	if( bts ) then
	  ...
	  do i=1,nvar
            call fem_file_read_data_mpi(iformat,iunit,nvers,np,lmax,nmpi,ibc,kranf,offset,string        &
     &            ,pinfo(id)%ilhkv_file,pinfo(id)%hd_file,nlvddi,pinfo(id)%data_file(1,1,i),ierr)
	    if( ierr /= 0 ) goto 99
	    if( string /= pinfo(id)%strings_file(i) ) goto 98
	  end do
	end if
   ...
   98	continue
	...
	write(6,*) 'new: ',string
	call iff_print_file_info(id)
	stop 'error stop iff_read_data_mpi'
   99	continue
	write(6,*) 'error reading data: ',ierr
	write(6,*) 'time: ',dtime
	call iff_print_file_info(id)
	stop 'error stop iff_read_data_mpi'
	end subroutine iff_read_data_mpi
...
!****************************************************************
	...
	if( nexp == nkn_fem ) then
	  do ivar=1,nvar
	      call intp_reg_nodes(nx,ny,x0,y0,dx,dy,flag,pinfo(id)%data_file(1,1,ivar),pinfo(id)%data(1,1,ivar,iintp),ierr)
	    !if( bneedall .and. ierr .ne. 0 ) goto 99	!is handled later
	  end do
	else if( nexp == nel_fem ) then
	  do ivar=1,nvar
	    call intp_reg_elems(nx,ny,x0,y0,dx,dy,flag,pinfo(id)%data_file(1,1,ivar),pinfo(id)%data(1,1,ivar,iintp),ierr)
	    !if( bneedall .and. ierr .ne. 0 ) goto 99	!is handled later
	  end do
	else
...
!****************************************************************
	...
	double precision, allocatable :: data(:,:,:)
	double precision, allocatable :: data2dreg(:)
	double precision, allocatable :: data2dfem(:)
	double precision, allocatable :: hfem(:)
	...
	call intp_reg_intp_fr(nx,ny,flag,pinfo(id)%hd_file,nexp,fr,hfem,ierr)	!interpolate depth from reg to fem

	do ivar=1,nvar
	  do l=1,lmax
	      data2dreg = pinfo(id)%data_file(l,:,ivar)
	    call intp_reg_intp_fr(nx,ny,flag,data2dreg,nexp,fr,data2dfem,ierr)
	...
	do ip=1,nexp
	  call iff_interpolate_vertical_int(id,iintp,lmax,hfem(ip),data(:,ip,:),ip)
	end do
...
! lmax must be 1
	...
	integer id
	integer iintp
	integer ip_from,ip_to,nexp

	double precision data(pinfo(id)%nvar)
...
! lmax must be 1
	...
	if( lexp <= 1 ) then		!2D
	  ...
	  ipl = ip_to
	  if(bmpi) then
	    if( pinfo(id)%nmpi /= nkn_fem ) ipl = pinfo(id)%nodes(ip_to)
	  else
...
! lexp/lfem must be 1
	...
	integer lmax,nexp
	double precision h
	double precision data(pinfo(id)%lmax,pinfo(id)%nvar)
...
! lexp/lfem must be 1
	...
	double precision value,hlayer
	double precision hl(pinfo(id)%lmax)
	...
	call get_layer_thickness(lmax,nsigma,hsigma,z,h,pinfo(id)%hlv_file,hl)
...
! global lmax and lexp are > 1
	...
	integer iintp
	integer lmax
	double precision h		!depth of data to be interpolated
	double precision data(pinfo(id)%lmax,pinfo(id)%nvar)
	integer ip_to
	...
	integer iu
	double precision acum,htot
	double precision z,hfem,hfile
	double precision hsigma
	double precision value,hlayer
	double precision hl(pinfo(id)%lmax)
	double precision hz_file(0:pinfo(id)%lmax+1)
	double precision val_file(pinfo(id)%lmax+1)
	double precision hl_fem(nlv_fem)
	double precision hz_fem(0:nlv_fem)
	double precision val_fem(nlv_fem)
	logical bdebug
	...
	call compute_sigma_info(lmax,pinfo(id)%hlv_file,nsigma,hsigma)
	call get_layer_thickness(lmax,nsigma,hsigma,z,hfile,pinfo(id)%hlv_file,hl)
	call get_bottom_of_layer(bcenter,lmax,z,hl,hz_file(1))
	hz_file(0) = z

	call compute_sigma_info(lfem,hlv_fem,nsigma,hsigma)
	call get_layer_thickness(lfem,nsigma,hsigma,z,hfem,hlv_fem,hl_fem)
...
!****************************************************************
        ...
        double precision value(ldim,ndim)
...
! this routine reads and interpolates new data - no parallel execution
        ...
        do while( tc < t )
          if(bmpi) then
            bok = iff_read_next_record_mpi(id,it)
          else
...
! does the final interpolation in time
        ...
        integer nintp,lexp,nexp,ilast,nmpi
        logical bonepoint,bconst,b2d,bmulti,bflag
        integer ipl,lfem,i,l,ip,j,iflag
        double precision val,tr,flag
        double precision time(pinfo(id)%nintp)
	!double precision time(pinfo(id)%nintp)
        double precision vals(pinfo(id)%nintp)
        ...
        tr = t          !double precision version of t
        iflag = 0
...
!****************************************************************
      ...
      subroutine regular_grid_min_max_index(id,nmpi)
      ...
      integer id,nmpi
      integer nx,ny
      double precision x0,y0,xn,yn
      double precision dx,dy
      double precision xmin,ymin,xmax,ymax
      double precision regpar(7)

      nx= pinfo(id)%regpar(1)
      ny= pinfo(id)%regpar(2)
      x0= pinfo(id)%regpar(3)
      y0= pinfo(id)%regpar(4)
      dx= pinfo(id)%regpar(5)
      dy= pinfo(id)%regpar(6)
      ...
      xmin= domain%nodes%xmin
      ymin= domain%nodes%ymin
      xmax= domain%nodes%xmax
      ymax= domain%nodes%ymax

      if (xmin .le. x0) then 
        iminreg= 1
      else if (xmin .ge. xn) then
        iminreg= nx- 1
      else
        iminreg= 1+ (xmin-x0)/ dx
      end if

      if (ymin .le. y0) then 
        jminreg= 1
      else if (ymin .ge. yn) then
        jminreg= ny- 1
      else
        jminreg= 1+ (ymin-y0)/ dy
      end if

      if (xmax .le. x0) then 
        imaxreg= 2
      else if (xmax .ge. xn) then
        imaxreg= nx
      else
        imaxreg= 2+ (xmax-x0)/ dx
      end if

      if (ymax .le. y0) then 
        jmaxreg= 2
      else if (ymax .ge. yn) then
        jmaxreg= ny
      else
        jmaxreg= 2+ (ymax-y0)/ dy
      end if


      nxreg=imaxreg-iminreg+1
      nyreg=jmaxreg-jminreg+1
      x0reg=(iminreg-1)*dx+x0
      y0reg=(jminreg-1)*dy+y0
      nmpi = (imaxreg-iminreg+1)* (jmaxreg-jminreg+1)

      return
 
      end subroutine

      subroutine read_regular_grid_mpi(id)

      use shympi

      implicit none
      integer iunit,id,nmpi
      integer k,i,j,h
      integer nvers
      !double precision data(30)!47*61
      character(20) format,format2
      !double precision,dimension(:),allocatable :: offset
      double precision offset(iminreg-1)!2 is = to imin-1
      character*80 text
      integer nvar,lmax
      double precision stampa
      ...
      do i=1,nvar
        ...
        do j=1,nint(pinfo(id)%regpar(2))
          if((j.ge.jminreg).and.(j.le.jmaxreg)) then
            read(iunit,*),(offset(h),h=1,size(offset)) ,(pinfo(id)%data_file(1,k,i),k =k,k+imaxreg-iminreg)
          else
            read(iunit,*)
          end if
        end do
...
! everything needed is in array (unit, vars etc...)
        ...
        double precision array(ndim)        !array with all information

	integer nv,id
	integer nexp,lexp
	integer nodes(1)
	double precision dtime
	double precision vconst(nvar)
	...
	if( ndim < 2 ) stop 'error stop exffil: ndim'
        ...
        if(bmpi) then
	  call iff_init_mpi(dtime,file,nvar,nexp,0,lexp,nintp,nodes,vconst,id,0)
        else
...
!****************************************************************
	...
	implicit none
        ...
        double precision array(*)           !array with information from set-up
        double precision t                  !t value for which to interpolate
        double precision rint(*)            !interpolated values
