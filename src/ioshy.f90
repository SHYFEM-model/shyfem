...
!==================================================================
	...
	type, private :: entry
	  ...
	  character*80 :: femver
          ...
          double precision, allocatable :: xgv(:)
          double precision, allocatable :: ygv(:)
          double precision, allocatable :: hm3v(:,:)
...
!******************************************************************
	...
	open(iunit,file=file,status='unknown',form='unformatted',iostat=ios)
	if( ios /= 0 ) return
...
!************************************************************
	...
	integer id
	double precision xgv(pentry(id)%nkn), ygv(pentry(id)%nkn)
	xgv = pentry(id)%xgv
	ygv = pentry(id)%ygv
...
!************************************************************
	...
	integer id
	double precision hm3v(3,pentry(id)%nel)
	hm3v = pentry(id)%hm3v
...
!************************************************************
	...
	integer id
	double precision hlv(pentry(id)%nlv)
	hlv = pentry(id)%hlv
...
!**************************************************************
	...
	if( lmax <= 1 ) then
	  ...
	  read(iunit,iostat=ierr) (( c(l,i),l=1,il(i) ),i=1,n )
	else
	  read(iunit,iostat=ierr) ((( c(l,j+m*(i-1)),l=1,il(i) ),j=1,m ),i=1,n )
	end if
	deallocate(il)
...
!**************************************************************
	...
	if( lmax <= 1 ) then
	  ...
	  write(iunit,iostat=ierr) (( c(l,i),l=1,il(i) ),i=1,n )
	else
	  write(iunit,iostat=ierr) ((( c(l,j+m*(i-1)),l=1,il(i) ),j=1,m ),i=1,n )
	end if
	deallocate(il)
