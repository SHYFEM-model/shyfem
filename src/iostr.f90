...
! writes info on total energy to info file
	...
	double precision kenergy,penergy,tenergy
        double precision kenergyx, kenergyy

	integer iuinfo
	save iuinfo
	data iuinfo / 0 /
        integer ilock
        logical blockexchange
	...
	!call energ3d(kenergy,penergy,-1)
	    ...
	    !call energ3d2(kenergyx,kenergyy,kenergy,penergy) !ivb
        else
	...
	if(shympi_is_master()) then
	  write(iuinfo,*)'energy(k,p,tot):',it,kenergy,penergy,tenergy
          if (blockexchange) then
	    write(iuinfo,*)'energy(kx,ky  ):',it,kenergyx,kenergyy
          end if
	end if
