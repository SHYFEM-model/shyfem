!
! $Id: lagrange_flux.f,v 1.3 2009-05-21 09:24:00 georg Exp $
!
...
! initializes length of element sides
	...
	double precision x1,x2,y1,y2,dx,dy,ddx,ddy,d
...
! sets up fluxes in 3d - has to be done every time step
	...
	integer itype
	double precision az,azpar
	double precision tdif
	
	integer elems(maxlnk)
	double precision rflux(nlvdi,maxlnk)       !fluxes across finite volume k
	double precision tflux(nlvdi,maxlnk)       !fluxes across sides of element
...
! sets up fluxes - has to be done every time step
	...
	integer elems(maxlnk)
	double precision rflux(maxlnk)       !fluxes across finite volume k
	double precision tflux(maxlnk)       !fluxes across sides of element
	double precision tflux_aux(maxlnk)   !fluxes across sides of element (aux)
	double precision flux2d_aux(3,nel)
...
! we do not use lkmax, but lmax
	...
	use levels
        ...
        integer k,lkmax,n
	double precision tflux(nlvdi,1)       !fluxes across sides of element
...
! computes fluxes in element
	...
	use geom
        ...
        integer k,n
        double precision tflux(1)
	double precision flux2d_loc(3,1)
        integer ne              !total number of elements in elems
        integer elems(ne)       !elements around k
...
! all this has to be revised for sigma layers
	...
	integer ie,ii,i1,i2
	integer l,lmax
	double precision flx,dh,zi1,zi2,zi3,dp,ar,dst
