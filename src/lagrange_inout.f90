!
! $Id: lagrange_inout.f,v 1.2 2009-03-11 16:25:59 georg Exp $
!
...
! 07.10.2015    mic	seed 3d between surface and l_bot 
!
!*******************************************************************
!-------------------------------------------------------------------------
        module lagrange_inout
!-------------------------------------------------------------------------
        contains
!-------------------------------------------------------------------------
...
! z = 0.5 (corresponding to larva in water) 
	...
	double precision rtime	!fraction of time step to be inserted (0 for all) [0-1]
	double precision x,y	!coordinates of particle to be inserted
	double precision z		!vertical (relative) coordinate [0-1]

	integer pt	!particle type
	double precision pc		!custom property
...
! 1 inserts at end of time step - no advection for this time step needed
	...
	call mod_lagrange_handle_alloc(nbdy)
        ...
        !tin(nbdy) = it - double precision(idt)*(1.-rtime)
        !tin(nbdy) = it				!old
...
! finds vertical position for depth hact
	...
	double precision hl(lmax)		!layer thickness
	double precision hact		!vertical depth of particle
	integer lb		!layer [1-lmax] (return)
	double precision z			!position in layer [0-1] (return, 1 bottom)
...
!   - curstom property
        ...
        use lagrange_sedim, only : lgr_set_sedim
	...
	logical, intent(in)           :: boilsim !true for oil module
        ...
        double precision, intent(out)             :: pc      !custom property
...
! mic : 15/10/2015 write in v.5  hl = effective absolute depth 
! mic : need to add more start info: level depth custom 
! mic : suggests to write for each release a small file_ini.lgr
	...
	double precision x,y,z,c,xst,yst,zst,tin
	double precision hl,ht,hr
	...
	do i=1,nbdy
	  ...
	  if( ie .ne. 0 ) then
	    if( nvers .eq. 3 ) then
	      ...
	      write(iu) id,x,y,z,ie,lb,hr,ty,c,xst,yst,zst,est,tin
	    else
	      write(6,*) 'nvers = ',nvers
...
! outputs particles as density (concentration) to NOS file

	use lagrange_data
	use lagrange_connect
	use evgeom
	use basin
        use conz_util
	...
	double precision area_node,z

        integer ecount(nel)
        integer kcount(nkn)
        double precision area(nkn)
        double precision density(nkn)
...
! tests compress routine
	...
	do i=1,nadd
	  ie = ie + 1
	  call insert_particle_surface(ie,0,0.d0,1.d0,1.d0)
	end do
