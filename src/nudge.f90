!
! $Id: baswork.f,v 1.20 2010-03-22 15:29:31 georg Exp $
!
...
!==================================================================
	...
	double precision, save :: tramp = 0.

	double precision, allocatable :: andg_data(:)		!data of observations
	...
	double precision, allocatable :: andg_dist(:)	!distance to be used
	double precision, allocatable :: andg_weight(:)	!weight to be used
	double precision, allocatable :: andg_obs(:)	!observations to be used
...
!****************************************************************
	...
	double precision talpha,ttau,t,zobs,zcontrib,w
	double precision rint(ndgdim)
	double precision zeta(ndgdim)
	double precision cont(ndgdim)
...
! sets up andg_dist and andg_weight
	...
	double precision ttau		!relaxation time
	double precision sigma		!sigma distance

	integer k,kk,iu
	double precision dx,dy,dist,dist2,w,s

	double precision d(nkn)
	double precision dw(nkn)
...
! sets up icol (local) and ndg_nodes and ndg_area
...
!*******************************************************************
	...
	double precision vconst(2)
	double precision uobs_surf(nel),vobs_surf(nel)
	...
	taudefvel = getpar('tauvel')
        ...
        if(bmpi) then
          call iff_init_mpi(dtime0,surffile,nvar,np,0,lmax,nintp,nodes,vconst,idsurf,0)
        else
          call iff_init(dtime0,surffile,nvar,np,lmax,nintp,nodes,vconst,idsurf)
        end if
...
!*******************************************************************
	...
	double precision h,tau,taudef
	double precision u,v,s,flag
...
!       ---------------------------------------------------------
	...
	string2 = adjustl(string2)
        ...
        if( string1 == ' ' ) then        !TS file or constant
          ...
          if( string1 == 'velocity x' .and. string2 == 'velocity y' ) then
            ivtype = 1
          else
