...
!****************************************************************
	...
	double precision rval(nlvddi,ndim)

	integer l,lmax,i,j
	double precision x(4),y(4),t
...
!****************************************************************
	...
	integer ie,ii,k,l,lmax
	integer ierr
	double precision utmax,umax,zmax,wmax,smax,tmax
	...
	do ie=1,nel
	  lmax = ilhv(ie)
	  do l=1,lmax
	    call off_check_val('ut',ie,l,ut(l,ie,ig),utmax,ierr)
	    call off_check_val('vt',ie,l,vt(l,ie,ig),utmax,ierr)
	  ...
	  do ii=1,3
	    call off_check_val('ze',ie,ii,ze(ii,ie,ig),zmax,ierr)
	  end do
	end do
	...
	do k=1,nkn
	  lmax = ilhkv(k)
	  do l=1,lmax-1
	    call off_check_val('wn',k,l,wn(l,k,ig),wmax,ierr)
	  end do
	  do l=1,lmax
	    call off_check_val('sn',k,l,sn(l,k,ig),smax,ierr)
	    call off_check_val('tn',k,l,tn(l,k,ig),tmax,ierr)
	  end do
	  call off_check_val('zn',k,0,zn(k,ig),zmax,ierr)
	end do
