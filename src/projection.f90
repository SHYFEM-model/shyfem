...
! transformation from (lon/lat) to utm (x,y)
        ...
        k3 = k0 * nu * sin(phi)*((cos(phi)**3)/24.) * (5. - tan(phi)**2 + 9.*ep2*cos(phi)**2 + 4.*ep2*ep2*cos(phi)**4)
        k4 = k0 * nu * cos(phi)
        k5 = k0 * nu * ((cos(phi)**3)/6.) * (1. - tan(phi)**2 + ep2*cos(phi)**2)
...
! transformation from utm (x,y) to (lon/lat)
        ...
        q4 = (61. + 90.*t1 + 298.*c1 + 45.*t1*t1 - 3.*c1*c1 - 252.*ep2) * d6 / 720.
        q5 = d
        q6 = (1. + 2.*t1 + c1) * d2*d / 6.
        q7 = (5. - 2.*c1 + 28.*t1 - 3.*c1*c1 + 8.*ep2 + 24.*t1*t1) * d4 * d / 120.
...
! conversion of coordinates
        ...
        double precision, intent(inout)  :: xc(n),yc(n)	! cartesian coordinates
        double precision, intent(inout)  :: xg(n),yg(n)	! geographical coordinates
...
!*******************************************************************
        ...
        double precision :: proj_ran_intv
        integer :: idum
