...
!=====================================================================
        ...
        integer, save :: iturb_rst = 0  !ivb      
        integer, save :: iheat_rst = 0  !ivb      
...
!-----------------------------------------------------------------
        ...
        iunit = ifileo(iunit,name,'unformatted','old')
        if( iunit .le. 0 ) then
        ...
        write(6,*) ' iturb_rst    ',iturb_rst     
        write(6,*) ' iheat_rst    ',iheat_rst     
...
! icode = 7	turbulent variables     !ivb
! icode = 8	meteo variables         !ivb
        ...
        else if( icode .eq. 7 ) then    !ivb
          rst_has_restart = iturb_rst .gt. 0
        else if( icode .eq. 8 ) then    !ivb
          rst_has_restart = iheat_rst .gt. 0
        else
          rst_has_restart = .false.
        end if
...
! example: iflag = 1011 means that for icode 1,2,4 function is true, else false
        ...
        rst_use_restart = rst_has_restart(icode) .and. rst_want_restart(icode)
...
!-----------------------------------------------------
        ...
        if( icall .eq. 0 ) then
          ...
          if( .not. bonce ) then
            if(shympi_partition_on_elements()) then
              iunit = ifemopp('.rst','unformatted','new')
            else
...
! writes one record of restart data
        ...
        use conz_common, only : cnv, conzv
        use geom_dynamic
        use ts
        use hydro_vel
        use hydro_admin
        ...
        date    = nint(dgetpar('date'))
        time    = nint(dgetpar('time'))
        iturb   = nint(getpar('iturb'))         !ivb
        iheat   = nint(getpar('iheat'))         !ivb        
        ...
        ! vertical mixing
        write(iunit) iturb      !ivb
        if (iturb > 0) then
          write(iunit) ((visv(l,k),l=0,nlv),k=1,nkn)
          write(iunit) ((difv(l,k),l=0,nlv),k=1,nkn)
          write(iunit) ((tken(l,k),l=0,nlv),k=1,nkn)
          write(iunit) ((eps(l,k),l=0,nlv),k=1,nkn)
          write(iunit) ((rls(l,k),l=0,nlv),k=1,nkn)
        end if

        ! drag coefficient / evaporation        
        write(iunit) iheat      !ivb
        if (iheat .eq. 8) then
          write(iunit) (windcd(k),k=1,nkn)
          write(iunit) (evapv(k),k=1,nkn)    
        end if
...
!*******************************************************************
        ...
        subroutine rst_skip_record(iunit,atime,it,nvers,nrec,nkn,nel,nlv,iflag,ierr)
...
! returns info on record in restart file and skips data records
        ...
        if( nvers .ge. 9) then  !ivb - turbulent viscosity/diffusivity
          read(iunit) iturb
          if (iturb .gt. 0) then
            iflag = iflag + 1000000
            read(iunit)
        ...
        if (nvers .ge. 9) then  !ivb - drag coefficient / evaporation
          read(iunit) iheat
          if (iheat .eq. 8) then
            iflag = iflag + 10000000
            read(iunit)
            read(iunit)
          end if      
        end if
...
!        if( nvers > 8 ) read(iunit) date,time
        read(iunit) date,time   !ivb - date/time written always in rst
        ...
        if( rst_want_restart(1) ) then
          ...
          read(iunit) ((utlnv(l,ie),l=1,newnlv),ie=1,nel)
          read(iunit) ((vtlnv(l,ie),l=1,newnlv),ie=1,nel)
        else
          read(iunit)
          read(iunit)
          read(iunit)
        ...
        ! active tracers / density
        read(iunit) ibarcl_rst
        if( ibarcl_rst .gt. 0 ) then
          if( rst_want_restart(3) ) then
            read(iunit) ((saltv(l,k),l=1,newnlv),k=1,nkn)
            read(iunit) ((tempv(l,k),l=1,newnlv),k=1,nkn)
            read(iunit) ((rhov(l,k),l=1,newnlv),k=1,nkn)
          else
            read(iunit)
            read(iunit)     !ivb - BUG?
            read(iunit)     !ivb - BUG?
        ...
        if( iconz_rst > 0 .and. .not. rst_want_restart(4) ) then
          ...
          read(iunit) ((cnv(l,k),l=1,newnlv),k=1,nkn)
        else if( iconz_rst .gt. 1 ) then
          call mod_conz_init(iconz_rst,nkn,nlvdi)
          read(iunit) (((conzv(l,k,i),l=1,newnlv),k=1,nkn),i=1,iconz_rst)
        end if
        ...
        if( iwvert_rst .gt. 0 ) then
          if( rst_want_restart(5) ) then
            read(iunit) ((wlnv(l,k),l=0,newnlv),k=1,nkn)
          else
            read(iunit)
          end if
        end if
        ...
        ! vertical mixing       !ivb
        read(iunit) iturb_rst
        if( iturb_rst .gt. 0 ) then
          if( rst_want_restart(7) ) then
            read(iunit) ((visv(l,k),l=0,newnlv),k=1,nkn)
            read(iunit) ((difv(l,k),l=0,newnlv),k=1,nkn)
            read(iunit) ((tken(l,k),l=0,newnlv),k=1,nkn)
            read(iunit) ((eps(l,k),l=0,newnlv),k=1,nkn)
            read(iunit) ((rls(l,k),l=0,newnlv),k=1,nkn)
          else
            read(iunit)
            read(iunit)
            read(iunit)
        ...
        ! drag coefficient / evaporation        !ivb
        read(iunit) iheat_rst
        if (iheat_rst .eq. 8) then
          if( rst_want_restart(8) ) then
            read(iunit) (windcd(k),k=1,nkn)
            read(iunit) (evapv(k),k=1,nkn)    
          else
            read(iunit)
            read(iunit)
          end if    
