...
! -------------------------------------------------------------
      ...
      double precision sedkpar,difmol
...
! -------------------------------------------------------------
      ...
      double precision bh(nkndim)		    	!bottom height variation [m]
      double precision tcn(nlvdim,nkndim) 		!total sediment concentration [kg/m3]
      double precision gskm(nkndim)			!AVERAGE SEDIMENT GRAIN DIAMETER ON NODES (M)
      double precision sbound(nsdim)                !boundary vector [kg/m3]
      double precision scn(nlvdim,nkndim,nsdim)     !suspended sediment conc (kg/m3)
      double precision eps(0:nlvdim,nkndim,nsdim)	!vertical mixing coefficient
      double precision thick			!initial thickness [m]
      double precision conref			!initial concentration [kg/m3]
      double precision wsink(0:nlvdim,nkndim,nsdim)	!settling velocity for suspended sediment
      double precision gdx(nkndim),gdy(nkndim)	!slope gradients
      double precision tao(nkndim)                  !wave-current shear stress
	double precision v1v(nkn)
      ...
      double precision sedpa(7)                     !sediment parameter vector
      common /sedpa/sedpa
      double precision gsc(nsdim)                   !grainsize class
      common /gsc/gsc
      ...
      double precision salref,temref		!salinity [psu] and temperature [C]

      character*10 what
      double precision tsec
      double precision totbed(nkndim)		!total bedload transport (kg/ms)
      ...
      parameter ( fact = 1. )
      double precision load(nlvdim,nkndim)
      integer itmsed,idtsed 		!output parameter
      integer ia_out1(4),ia_out2(4)

! function
      integer iround
      double precision sload(nkndim,nsdim)	  !suspended sediment load [kg/s]
...
!         --------------------------------------------------
          ...
          what = 'sedt'
          if(bmpi) then
            call bnds_init_mpi(what,dtime0,nintp,nvar,nkn,nlvmax,sbound,idsedi)
          else
            call bnds_init_new(what,dtime0,nintp,nvar,nkn,nlv,sbound,idsedi)
          end if
...
!$OMP DO SCHEDULE(DYNAMIC)
	  ...
	  call load3d(sload(1,is),nkn,nlvdi,ilhkv,load)
          call scal_adv_fact(what,is,fact,scn(1,1,is),idsedi,sedkpar    &
     &          ,fact,wsink(0,1,is),fact,load,difhv,eps(0,1,is),difmol)
        end do
...
! --- output variables
        ...
        integer npi			!
        integer nps
        double precision prin(0:nsdim,nsdim)        !initial percetage [0,1]
        common /prin/prin
        integer nrst			!number of sediment tipe
        double precision tue(nsdim)			!initial erosion threshold
        common /tue/tue        
        save /gsc/
        save /sedpa/
...
!			sediment concentration.
        ...
        call addpar('idtsed',0.d0)
        call addpar('itmsed',-1.d0)
...
!		the simulation (Default 0).
        ...
        call addpar('adjtime',-99999999.d0)
...
!		of grainsize classes per the number of area types. \\
!		|example: percin = 0.25 0.25 0.25 0.25  \\
!			 	   0.20 0.20 0.30 0.30  \\
!				   0.45 0.15 0.15 0.15| \\

        call addpar('percin',0.d0)
...
! vertical stratification). 
	...
	use basin
        ...
        double precision prin(0:nsdim,nsdim)                !initial percetage [0,1]
        common /prin/prin
        double precision tue(nsdim)             		!initial erosion threshold/density
        common /tue/tue
        double precision gs(nsdim)              !sediment grain diameter (m)
        double precision percbd(nlbdim,nsdim,nkndim)        !fraction of sediment [0,1]
        double precision bedn(nlbdim,3,nkndim)  !bed characteristics in 3 column table
        ...
        integer nbed				!initial number of bed layers
        double precision thick				!initial thickness [m]
        double precision gskm(nkndim)			!average grainsize per node [m]
	...
	double precision ptot
        ...
        double precision pers(nkndim,nsdim)			!percbd initialized from file
        double precision tuek(nkndim,1)			!tauce initialized from file
        integer npi
...
! --- input variables
	...
	double precision z0bk		!bottom roughness length from sedtrans [m]
...
! --- local variables
	...
	double precision br 	  	!bottom roughness height [m]
	double precision z0   		!bottom roughness length [m]
...
! --- input variables
        double precision u,v		!x,y components
...
!         -4- Diffusivity from GOTM 
        ...
        subroutine vmixcoef(dtype,l,dep,h,wsink,ustc,ustcw,ub,dcw,ht,per,dx,difv,visv,eps)
...
! --- input variables
	...
	integer dtype			!type of diffusion algoritm
        ...
        double precision dep(nlvdim)		!thickness of layer [m]
        ...
        double precision visv(0:nlvdim)		!water viscosity [from GOTM] 
        double precision difv(0:nlvdim)		!water diffusivity [from GOTM] 

! --- output variables

        double precision eps(0:nlvdim)
...
! SUBROUTINE SED_LOOP
     ...
     &	scn,eps,sedx,sedy,gdx,gdy,tao,gskm,percbd,bedn,salref,temref,wsink,sload,sflx)
...
! -------------------------------------------------------------
	...
	integer lmax			!bottom level
        ...
        double precision scn(nlvdim,nkndim,nsdim)   !cohesive suspended sediment conc (kg/m3)
        double precision riph(nkn)	!ripple height [m]
        double precision ripl(nkn)	!ripple length [m]
        double precision gskm(nkn)		!AVERAGE SEDIMENT GRAIN DIAMETER ON NODES (M)
        double precision eps(0:nlvdi,nkn,nsdim)	!vertical mixing coefficient
        double precision u,v			!x and y current components
        double precision gdx(nkn),gdy(nkn)	!slope gradients
        double precision tao(nkn)		!wave-current shear stress
        ...
        double precision bedn(nlbdim,3,nkn)	!bed characteristics in 3 column table
                                                ...
                                                ! (2) critical erosion stress (Pa)
                                                ! (3) dry bulk density (kg/m**3)
        double precision scns(nsdim)    	!suspended sediment conc (kg/m3)
	double precision hzoff
	double precision z0b
	double precision totbed(nkn)			!total bedload transport (kg/ms)
        double precision salref,temref			!salinity [psu] and temperature [C]
        double precision wsink(0:nlvdi,nkn,nsdim)       !settling velocity for suspended sediment

! -------------------------------------------------------------
! fem variables
! -------------------------------------------------------------

        double precision salt,temp			!salinity [psu] and temperature [C]
...
! cohesive
        ...
        double precision sload(nkn,nsdim)	!suspended sediment load [kg/s]
        double precision sloads(nsdim)	  	!suspended sediment load [kg/m²s]

	double precision ukbot(nkn),vkbot(nkn)
	double precision ddl(nkn)
...
!         -------------------------------------------------------------------
     ...
     &  lmax,eps,tao(k),Z0,sloads,sflx(1,k))
	  ...
	  z0bk(k) = max(z0b,z0bmin)		!GGUZ0
          ...
          do is = 1,nscls
	    totbed(k) = totbed(k) + sqrt(sedx(is,k)**2 + sedy(is,k)**2)* bedn(1,3,k)
...
! ------------ OUTPUT VARIABLES -----------------
        ...
        double precision ws(nsdim)      !SETTLING VELOCITY FOR NON-COHESIVE SEDIMENT (M/SEC)
        double precision tao			!wave-current shear stress
	double precision sload(nsdim)	  	!suspended sediment load [kg/m²s]
...
! ------------ LOCAL VARIABLES -----------------
        ...
        double precision gdx,gdy			!slope gradients
...
!       -------------------------------------------------------------------
        ...
        do is = 1,nsclsc
          CALL THRESH(VISK,gs(is),RHOS,RHOW,ws(is),usb(is),uss(is),ust(is),dxx(is))
	  WSI(is) = ws(is)
        end do
...
!       -------------------------------------------------------------------
	...
	dtype = 2
     ...
     &	       deltacw,ht,per,dxx(is),difv(1,k),visv(1,k),eps(1,k,is))
        end do
...
!         -------------------------------------------------------------------
	  ...
	  blimit = .true.
          if( tau0.GT.BEDCHA(1,2) .OR. ( tau0.EQ.BEDCHA(1,2) .AND. tau0.GE.BEDCHA(2,2) ) ) THEN
...
20          NBED=I-1
            ...
            CALL EROS1(NBED,nlbdim,BEDCHA,tau0,MAXEMASS,EMASS,bmix,TIMEDR,TAUCE)
...
! ------------ OUTPUT VARIABLES -----------------
	double precision sload(nsdim)	  	!suspended sediment load [kg/m2s]
...
20        NBED=I-1
          ...
          CALL EROS1(NBED,nlbdim,BEDCHA,TAU0,MAXEMASS,EMASS,ZS,TIMEDR,TAUCE)
...
!       -------------------------------------------------------------------
        ...
        if(TCONC1.gt.0.d0) then 
          call depos(DL,TIMEDR,NBCC,CONC,TAU0,RHOW,TCONC1,TCONC2,VISK,DMASS)
	  
          do is = 1,nbcc
	    sload(is) = sload(is) - dmass * pers(is) / TIMEDR
	    sflux(is) = sflux(is) + DMASS*2.d0/(RHOMUD+BEDCHA(1,3))*pers(is)
	  end do
        end if
...
! ********************************************************************
	...
	implicit none
        ...
        double precision bh(nkndim)				!bed elevation
...
! --- input variables
        ...
        double precision gdx,gdy			!depth gradient in x and y
...
! or deposited
        ...
        subroutine bedman(gs,nscls,timedr,bflux,sflux,bh,gskm,bdh,percbd,bedn)
	...
	use basin, only : nkn,nel,ngr,mbw
        ...
        double precision bh(nkndim)				!bed elevation
        double precision gskm(nkndim)			!average sediment grainsize in node k
...
!         -------------------------------------------------------------------
     ...
     &  percbd(2,is)*(BEDCHA(3,1)-BEDCHA(2,1)))/(BEDCHA(3,1)-bmix)
          ...
          BEDCHA(2,3) = lin(d0,BEDCHA(2,1),BEDCHA(1,3),BEDCHA(2,3),bmix)
          BEDCHA(2,2) = lin(d0,BEDCHA(2,1),BEDCHA(1,2),BEDCHA(2,2),bmix)
          BEDCHA(2,1) = bmix
...
!       -------------------------------------------------------------------
        ...
        do is = 1,nscls
          percbd(1,is) = (percbd(1,is)*BEDCHA(2,1) + percbd(2,is)*(bmix-BEDCHA(2,1)))/bmix
        end do

        BEDCHA(2,3) = lin(BEDCHA(2,1),BEDCHA(3,1),BEDCHA(2,3),BEDCHA(3,3),bmix)
        BEDCHA(2,2) = lin(BEDCHA(2,1),BEDCHA(3,1),BEDCHA(2,2),BEDCHA(3,2),bmix)
        BEDCHA(2,1) = bmix
...
! ********************************************************************
        ...
        subroutine depbed(iss,nlbd,nscls,BEDCHA,percbd,flux,bnet,btot,gs)
...
!         -------------------------------------------------------------------
	  ...
	  BEDCHA(3,1) = BEDCHA(4,1) - blimit
          BEDCHA(2,2) = lin(d0,BEDCHA(3,1),BEDCHA(1,2),BEDCHA(3,2),BEDCHA(2,1))
          BEDCHA(2,3) = lin(d0,BEDCHA(3,1),BEDCHA(1,3),BEDCHA(3,3),BEDCHA(2,1))
...
! ********************************************************************
        ...
        subroutine erobed(iss,nlbd,nscls,BEDCHA,percbd,flux,bnet,btot,bpre)
...
! Compute the total node concentration
	...
	double precision rhos,conc
        integer nscls				!number of grainsize class
        double precision scn(nlvdim,nkndim,nsdim)  		!suspended concentration
        double precision tcon(nlvdim,nkndim)		!total concentration
...
! Update the element depth in function of erosion/deposition
	...
	double precision v1v(nkn)
        ...
        double precision dh
        double precision evdep				!element depth variation
        integer ie,ii,k,iw

	double precision bdhrst(nkn)
	integer nkk,nv,iv
        character*80 name
	integer icall
        save icall
...
! or if bed slope is greater than angle of repose
	...
	use basin, only : nkn,nel,ngr,mbw
        ...
        double precision kvalue(nkndim)             !variable
        ...
        double precision area
        double precision baux(nkndim)
        double precision saux,aaux,smm
...
! parameters
	include 'param.h'
! arguments
	double precision vv(nkndim)
	double precision vv1(nkndim)
! common



! local
	double precision ukbot(nkndim),vkbot(nkndim)
	double precision u,v
	double precision ddl(nkndim)
	integer ie,lmax,k,ii
	double precision aj,vol,depth
	integer nsigma
...
!******************************************************************
	...
	double precision sload(1)
	integer nkn
	integer nlvdi
        integer ilhkv(1)             !number of element and node level

	double precision load(nlvdi,1)
