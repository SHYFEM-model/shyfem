...
!          DX = DIMENSIONLESS GRAIN SIZE
      ...
      IF( .NOT. COHES ) CALL THRESH(VISK,GD,RHOS,RHOW,FALL,USTCRB,USTCRS,USTUP,DX)
...
! FOR SHEET-FLOW, RIPPLE HEIGHT AND LENGTH WILL BE 0
           ...
           ELSE
              ...
              CALL CURRIPPLE(GD,RHOS,RHOW,VISK,USTCS,USTCRB,USTUP,RHINP,RLINP,RHEIGHT,RLENGTH)
          ENDIF
...
!*************************
      ...
      PHI100=DMIN1(ABS(CDIR-WDIR),ABS(180.-ABS(CDIR-WDIR)),360.-ABS(CDIR-WDIR))*ASIN(1.)/90.
      PHIB=PHI100
...
! FOR COHESIVE SEDIMENTS USE COSTANT BED ROUGHNESS HEIGHT.
      ...
      CALL FRIC2(UZ,Z,PHI100,UB,PER,GKB,RKBC,FCW,UA,PHIB,U100,USTCS,USTWS,USTCWS,DELTACW)
...
! COMPUTE INITIAL BED ROUGHNESS HEIGHTS BASED ON SKIN-FRICTION UST'S
     ...
     &  USTUP,GD,RHOW,RHOS,RHEIGHT,RLENGTH,SKB,HTM,TKB,USTCWSE,RPLCOEF,USTBF)
...
! ROUGHNESS HEIGHTS, SKB.
      CALL FRIC2(UZ,Z,PHI100,UB,PER,SKB,RKBC,FCWB,UA,PHIB,U100,USTCSB,USTWSB,USTCWSB,DELTACW)
...
! COMPUTE TOTAL FCW (FBAD) AND UST'S BASED ON TOTAL ROUGHNESS HEIGHT TKB.
      CALL FRIC2(UZ,Z,PHI100,UB,PER,TKB,RKBC,FBAD,UA,PHIB,U100,USTC,USTW,USTCW,DELTACW)
...
!---------------------------------------------------------------------------
      ...
      SUBROUTINE CURRIPPLE(GD,RHOS,RHOW,VISK,USTCS,USTCRB,USTUP,RHINP,RLINP,RHEIGHT,RLENGTH)
...
!---------------------------------------------------------------------------
      ...
      SUBROUTINE FRIC2(UZ,Z,PHI100,UB,PER,RKB,RKBC,FCW,UA,PHIB,U100,USTC,USTW,USTCW,DELTACW)
...
! GET FINAL BED ROUGHNESS AND BED ROUGHNESS HEIGHT USING LOG LAW
      ...
      CRNEW=SQRT(1.0+2*(USTC/USTW)**2*COS(PHIB)+(USTC/USTW)**4)
...
!  COMPUTE WAVE-DOMINANT RIPPLES FOR U*WS/U*CS>=1.25
           IF(USTWS/USTCS .GE. 1.25) THEN
                STEEP=0.15
!                RHEIGHT=GD*(9.86*(USTCWS/USTCRB)+37.91)
...
!---------------------------------------------------------------------------
     ...
     &  USTUP,Z0C,DELTACW,USTCS,USTWS,USTCWSB,USTC,USTW,RPLCOEF,USTBF,Z0,GD,DX,RHOS,QS,QSDIR,C0,C0A)
...
! INITIALIZE ZZ, CZ AND UUZ
      DATA ZZINI /0.0,0.01,0.02,0.03,0.05,0.07,0.1,0.2,0.3,0.5,0.7,1.0,2.0,4.0,6.0,8.0,12.0,15.0,20.0/
...
! modified Smith and McLean (1977) at Z0S (see Sedtrans96).
      ...
      SUBROUTINE GETC0_SM(TAOCRB,RHOW,G,S,GD,UST,USTUP,TAOST,USTBF,C0,Z0S)
...
!---------------------------------------------------------------------------
     ...
     &  USTUP,PHIB,PHI100,USTCS,USTWS,USTCWSB,RPLCOEF,TB1,TB2,TS1,      &
     &  PERBED,PERSUSP,USTBF,FCW,USTCWS,HT,CDIR,WDIR,IOPT1,DX,FALL,SED,SEDM,SEDDIR)
...
!-----------------------------------
 ...
 4    CALL YALIN(RHOW,RHOS,GD,USTCS,USTWS,USTCWS,USTCWSB,               &
     &  PHIB,USTCRB,USTBF,USTUP,RPLCOEF,W,NC,NT,TIMET,STEPC,STEPT,      &
     &  RLWRT,UB,FCW,UA,VCB,TAOCRB,G,DRHO,IOPT1,PER,PHI100,CDIR,WDIR,SED,SEDDIR)
      GOTO 998

 5    CALL VANRIJN(RHOW,RHOS,HT,D,DX,GD,USTCS,USTWS,USTCWS,     &
     &  USTCWSB,TAOCRB,PHIB,USTCRB,USTBF,USTUP,RPLCOEF,W,NC,NT, &
     &  TIMET,STEPC,STEPT,RLWRT,UB,TAO0,IOPT1,PER,PHI100,CDIR,WDIR,G,SED,SEDDIR)
      GOTO 998
...
!---------------------------------------------------------------------------
     ...
     &  USTCWSB,PHIB,USTCRB,USTBF,USTUP,RPLCOEF,W,NC,NT,TIMET,  &
     &  STEPC,STEPT,RLWRT,UB,FCW,UA,VCB,TAOCRB,G,DRHO,IOPT1,PER,PHI100,CDIR,WDIR,SED,SEDDIR)
...
! IF NO TRANSPORT OCCURRED UNDER WAVE TROUGH, EXIT
      ...
      DO 222 I = 0, (NT-1)
         TAOCWSX=TAOCSX+0.5*TAOWS*(COS(W*(RLWRT+I*STEPT))+COS(W*(RLWRT+(I+1)*STEPT)))
         TAOCWS=SQRT(TAOCSY**2+TAOCWSX**2)
         TAOCWSE=(RPLCOEF**2)*TAOCWS
...
! ************* CONSTANTS TRANSMITTED BY BLOCK *************
...
! surface critical erosion stress (BEDCHA(1,2)
     ...
     &   ( TAU0.GE.BEDCHA(2,2) .OR. NBED.EQ.1) ) ) THEN
         ...
         CALL EROS1(NBED,MAXBED,BEDCHA,TAU0,MAXEMASS,EMASS,ZS,TIMEDR,TAUCE)
...
! Compute deposition only if there are suspended sediments
      ...
      IF (TCONC1.EQ.0d0) THEN
         TCONC=0.D0
      ELSE
         CALL DEPOS(D,TIMEDR,NBCONC,CONC,TAU0,RHOW,TCONC1,TCONC,VISK,DMASS)
...
!                      compute eroded mass EMASS, which is used later
      ...
      SUBROUTINE EROS1(NBED,MAXBED,BEDCHA,TAU0,MAXEMASS,EMASS,ZS,TIMEDR,TAUCE)
...
! with TAUCES = Tce(top), for the second TAUCES = (Tce(top)+Tce(dz))/2
         ...
         DO 100, I=1,2
            ...
            IF(TAU0.LT.TAUCE)THEN ! check if erosion went to deep (into too strong sediments
               ZS=LIN(BEDCHA(IB,2),BEDCHA(IB+1,2),BEDCHA(IB,1),BEDCHA(IB+1,1),TAU0)
               TAUCE=TAU0
               RHOS=LIN(BEDCHA(IB,1),BEDCHA(IB+1,1),BEDCHA(IB,3),BEDCHA(IB+1,3),ZS)
...
! Compute the eroded mass, EMASS
      ...
      IF(ZS.GT.BEDCHA(NBED,1) .OR. ZS.EQ.BEDCHA(IB,1)) THEN
         DO 140, I=1,IB-1
            EMASS=EMASS + (BEDCHA(I+1,1)-BEDCHA(I,1))*(BEDCHA(I,3)+BEDCHA(I+1,3))*0.5d0   ! dZ * (RHOS1+RHOS2)/2
140      CONTINUE
...
20          CONTINUE
30          TAUCE=LIN(BEDCHA(IB-1,1), BEDCHA(IB,1),BEDCHA(IB-1,2), BEDCHA(IB,2), ZS)
            RHOS= LIN(BEDCHA(IB-1,1), BEDCHA(IB,1),BEDCHA(IB-1,3), BEDCHA(IB,3), ZS)
            MOVEZ=IB-2
         ENDIF
! Modify BEDCHA
         BEDCHA(1,2)=TAUCE
...
!             - compute TAUCD (include floccultion effects)
      ...
      SUBROUTINE DEPOS(D,TIMEDR,NBCONC,CONC,TAU0,RHOW,TCONC1,TCONC2,VISK,DMASS)
...
! An alternative would be to use Andy Manning's formula which include also turbulence
     ...
     &         DE**3 * 9.81d0*(RHOFLOC-RHOW)/(RHOW*VISK**2) )**0.5d0-10.36d0)
...
! COMPUTE THE CONCENTRATION REMAINING IN SUSPENSION AFTER DEPOSITION DURING TIME TIMEDR
               CONC(I)=CONC(I) * EXP(-WS*(1D0-TAU0/TAUCD)*(1d0-PRS)*TIMEDR/D)
...
! Uses P. Cozette's formulae (MSc thesis, 2000, SOC)
      ...
      ELSE
         ...
         ELSE
            TAOS=AULVA*CSULVA*(TRULVA-TAU0)/(0.2d0*TRULVA)*(0.8d0*TRULVA-TMULVA) ! partial resusponsion
         ENDIF
...
! ************* CONSTANTS TRANSMITTED BY COMMON BLOCK *************
      INCLUDE "sed_param.h"  ! declaration of the common blocks
...
!     C and E : define the shape (in conjunction with B and C)
         FINAL=DPROFA-DPROFB*EXP(-DPROFC*ABOVE(IB))-DPROFD*EXP(-DPROFE*ABOVE(IB))
...
! (RHO2-RHOfinal) = (RHO1-RHOfinal) * exp(-a*dt)
            BEDCHA(IB,3)=FINAL +(BEDCHA(IB,3)-FINAL) * exp(-CONSOA*TIMEDR)
         ENDIF
...
! If the computed TAUCE is smaller than the previous one, keep the previous one
         ...
         TAUCE=TEROA*(BEDCHA(IB,3)**TEROB)*(1+TEROC*(1-EXP(TEROD*ABOVE(IB))))
...
! Adapt the depth Z to the new densities by conserving the sediment mass
      DO 200, IB=2,NBED
         DZNEW=DZOLD(IB)*((OLDRHO(IB)+OLDRHO(IB-1)) / (BEDCHA(IB,3)+BEDCHA(IB-1,3)))
...
!   0 < TAUCE   and    0 < RHOS < 4000 kg/m**3
      DO 30, I=1,NBED
         IF (BEDCHA(I,2).LE.0 .OR. BEDCHA(I,3).LE.0 .OR.BEDCHA(I,3).GE.4000) GOTO 90
30    CONTINUE
...
10       MMID=(M1+M2)/2d0
         ...
         BEDCHA(IB,2)=TEROA*(BEDCHA(IB,3)**TEROB)*(1+TEROC*(1-EXP(TEROD*ABOVE)))
20    CONTINUE
...
! If NB is a positive double precision smaller than 0.5, then the layer thinner than
...
! for position z2, (dX-d1)/(z2-z1) = (d3-d1)/(z3-z2)
         DIFF(I)=(BEDCHA(I,1)-BEDCHA(I-1,1)) /(BEDCHA(I+1,1)-BEDCHA(I-1,1)) *   &
     &               (BEDCHA(I+1,3)-BEDCHA(I-1,3)) + BEDCHA(I-1,3) - BEDCHA(I,3)
40    CONTINUE
...
! decide whether upper or limit of layer has to be removed
         IF(P.EQ.1 .OR. ( DIFF(P).GE.DIFF(P+1) .AND. P+1.LT.NBED ) ) THEN
            P=P+1
         ENDIF
...
! X = r2 + (r1-r3)*z2/z3
         BEDCHA(1,3)=BEDCHA(2,3) + (BEDCHA(1,3)-BEDCHA(3,3))*BEDCHA(2,1)/BEDCHA(3,1)
      ELSE
...
! X = ( r2*(z3-z1) + r3*(z4-z2) + r4*(z2-z3) ) / (z4-z1)
     ...
     &         BEDCHA(P+1,3)*(BEDCHA(P-1,1)-BEDCHA(P,1)) ) / (BEDCHA(P+1,1)-BEDCHA(P-2,1))
      ENDIF
! reshuffle the rows
      DO 70, I=P,NBED-1
...
100   FORMAT (/'The existing file ',A,' will be overwritten, '/,'do you want to continue ? (Y/N)')
...
121   FORMAT('CSULVA TMULVA TRULVA RKERO E0 CDISRUPT CLIM1 CLIM2 ','KFLOC MFLOC ',      &
     &  'RHOCLAY CTAUDEP PRS RHOMUD DPROFA DPROFB DPROFC DPROFD DPROFE ',               &
     &  'CONSOA TEROA TEROB TEROC TEROD CDRAGRED Z0COH FCWCOH ','WSCLAY DOCOMPACT')
140   FORMAT('Following default parameters for the cohesive algorithm',' were modified')
...
! SUBROUTINE INICONST
!
! Initialize the constants in the common block CCONST.
...
!      CDISRUPT=0.001d0  ! calibrated for Sea Carousel Venice winter 99
      CDISRUPT=0.001d0
...
! Switch for calling performing compaction at each call to COHESIVE
