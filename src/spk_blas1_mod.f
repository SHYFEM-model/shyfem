...
c   20    go to next,(30, 50, 70, 110)
   ...
   20 continue
	if( next .eq. 30 ) then
		...
		stop 'error stop dnrm2: internal error (1)'
	end if
...
c
c
c     for double precision or d.p. set hitest = cuthi/n
...
c
c     applies a plane rotation, where the cos and sin (c and s) are double precision
...
c
c     scales a complex vector by a double precision constant.
c     jack dongarra, linpack, 3/11/78.
c
      complex cx(1)
      double precision sa
...
c
      nincx = n*incx
      do 10 i = 1,nincx,incx
        cx(i) = cmplx(sa*dble(cx(i)),sa*aimag(cx(i)))
   10 continue
      return
c
c        code for increment equal to 1
