...
c           EARLIER VERSIONS. 
c
c droptol = double precision. Sets the threshold for dropping small terms in the
...
c w       = double precision work array of length n+1.
c  
c----------------------------------------------------------------------
...
c=============
c jw      = integer work array of length 2*n.
c w       = double precision work array of length n 
c
c IMPORTANR NOTE:
c --------------
...
c-----------------------------------------------------------------------
      implicit none 
      integer n
      double precision a(*),alu(*),w(2*n),tol, alph 
...
c=============
c jw      = integer work array of length 2*n.
c w       = double precision work array of length 2*n 
c
c Notes:
c ------
...
c           lfil must be .ge. 0 
c
c tol     = double precision. Sets the threshold for dropping small terms in the
c           factorization. See below for details on dropping strategy.
c  
...
c-----------------------------------------------------------------------
c     does a quick-sort split of a double precision array.
c     on input a(1:n). is a double precision array
