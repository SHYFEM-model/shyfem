!*==runrc.spg  processed by SPAG 6.50Rc at 22:35 on  9 Feb 2009
      ...
      double precision Fpar(16) , Rhs(N) , Sol(N) , Guess(N) , Wk(*) , A(*) , Au(*)
      EXTERNAL SOLVER
!-----------------------------------------------------------------------
...
!     double precision dtime, dt(2), time
!     external dtime
      EXTERNAL DNRM2
      SAVE its , res
...
!
!         write (iou, *) its, double precision(res)
...
!            print *, 'Iterative sovler is facing a break-down.'
         ...
         ENDIF
!     time = dtime(dt)
!      write (iou, *) ipar(7), double precision(fpar(6))
...
!
      FUNCTION HFUN(X,Y,Z)
      double precision HFUN , X , Y , Z , GAMmax , GAMmay , ALPha
      COMMON /FUNC  / GAMmax , GAMmay , ALPha
