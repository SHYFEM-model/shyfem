...
c      integer n, ipar(16)
c      double precision rhs(n), sol(n), fpar(16), w(*)
c     Where
...
c
c     double precision function distdot(n,x,ix,y,iy)
c     integer n, ix, iy
c     double precision x(1+(n-1)*ix), y(1+(n-1)*iy)

c     This interface of DISTDOT is exactly the same as that of
c     DDOT (or SDOT if double precision == real*8) from BLAS-1. It should have
...
c      fpar(5) = sqrt(double precision(ipar(7)+1)) * tao
...
c
      ...
      double precision alpha,beta,psi,c,s,distdot
      logical lp,rp,full
...
c
      ...
      parameter(one=1.0D0,zero=0.0D0)
c
      double precision t,sqrt,distdot,ss,res,beta,ss1,delta,x,zeta,umm
...
c-----------------------------------------------------------------------
      ...
      double precision fpar(16), r(n), delx(n), sx, distdot
      external distdot
c-----------------------------------------------------------------------
...
c               -3: input vector is a linear combination of others
c
c     External routines used: double precision distdot
c-----------------------------------------------------------------------
      integer i,k
      double precision  nrm0, nrm1, fct, thr, distdot, zero, one, reorth
