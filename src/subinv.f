c
c $Id: subinv.f,v 1.4 2008-04-24 09:16:52 georg Exp $
c
c matrix inversion routines (non symmetric band matrix)
c
c contents :
c
c sgbco			factors and solves
c sgbfa			factors and solves (without condition number)
c sgbsl			solves the double precision band system using factorization
c
c dgbco			factors and solves
c dgbfa			factors and solves (without condition number)
c dgbsl			solves the double precision band system using factorization
c
...
c     sgbco factors a double precision band matrix by gaussian
