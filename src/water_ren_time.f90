!
! $Id: subwrt.f,v 1.58 2010-03-08 17:46:45 georg Exp $
!
...
! 28.02.2012	ggu&deb	completely restructured
...
! df/da = 0 => sum_i (y(i)-at(i)) = 0 => a = sum_i(y(i))/sum_i(t(i)) 
! and finally: tau = sum_i(t(i))/sum_i(y(i)) = sum_i(t(i))/sum_i(log(c0/c(i)))
!
!------------------------------------------------------------

!==================================================================
        module water_ren_time
!==================================================================
	...
	!double precision cvres3(nlvdi,nkn)      	!computed RT 3D
	!double precision vol(nlvdi,nkn)      		!volume
	!double precision cvacu(nlvdi,nkn)   !conz integrated
	!double precision volacu(nlvdi,nkn)  !volume integrated

	double precision, save, allocatable :: cvres3(:,:)
	double precision, save, allocatable :: vol(:,:)
	double precision, save, allocatable :: cvacu(:,:)
	double precision, save, allocatable :: volacu(:,:)

	double precision, save, allocatable :: rinside(:)
...
!==================================================================
	...
	integer iconz
	double precision c0
	double precision ctop,ccut
	double precision percmin

        double precision conz,perc,rcorrect
...
!------------------------------------------------------------
	  ...
	  if( bcompute ) then	!compute new renewal time
	    ...
	    call acu_comp(ia_out,blog,badj,it,c0,ccut,rcorrect,tacu,cvacu,cnv,cvres3)
...
! resets concentration for start of new computation
	...
	double precision c0				!concentration for nodes inside domain
	double precision cnv(nlvdi,nkn)
	double precision rinside(nkn)		!flag if node is inside domain
...
! simulates stirred tank
	...
	double precision c0				!concentration to impose
	double precision cnv(nlvdi,nkn)
	double precision rinside(nkn)		!flag if node is inside domain
...
! computes masses for different areas
	...
	double precision v,conz,area,hdep
...
! limits concentration between 0 and c0
	...
	double precision c0
	double precision cnv(nlvdi,nkn)		!concentration
...
! computes mass and volume on internal nodes
	...
	double precision cnv(nlvdi,nkn)		!concentration
	double precision rinside(nkn)		!flag if node is inside domain
	double precision vol(nlvdi,nkn)		!volume on node (return)
	double precision mass,volume	!total mass/volume (return)

	integer k,l,lmax
	double precision v,conz
...
!***************************************************************
	...
	integer it
	double precision c0					!value used for initialization
	double precision cnv(nlvdi,nkn)
	double precision vol(nlvdi,nkn)
	double precision rinside(nkn)
	double precision cvacu(nlvdi,nkn)
...
!***************************************************************
	...
	subroutine acu_comp(ia_out,blog,badj,it,c0,ccut,rcorrect,tacu,cvacu,cnv,cvres3)
...
! compute renewal time and write to file
	...
	integer it
	double precision c0
	double precision ccut
	double precision rcorrect
	double precision tacu
	double precision cvacu(nlvdi,nkn)		!accumulated conz
	double precision cnv(nlvdi,nkn)				!last concentration
	double precision cvres3(nlvdi,nkn)				!computed RT 3D
...
! write histogram
	...
	integer iu
	integer it
	double precision ctop			!cut at this value of renewal time
	double precision rinside(nkn)		!point is intern
	double precision cvres3(nlvdi,nkn)
	double precision volacu(nlvdi,nkn)
	...
	double precision conz,c,amax,cmax
	double precision v,dw,val
