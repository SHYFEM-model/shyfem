...
! DOCS  START   S_wave
!
! SHYFEM could be coupled with the spectral wind wave unstructured
! model WWMIII or computes wave characteristics using empirical
...
! \begin{itemize}
! \item significant wave height [m], variable 31
! \item mean wave perios [s], variable 32
! \item mean wave direction [deg], variable 33
! \end{itemize}
!
...
!   \item water level
!   \item bathymetry e number of vertical layers
...
! local
        ...
        double precision wavejb(nkndim)		!wave pressure
        double precision wtauw(nkndim)     		!wave supported stress
        double precision wavewl(nkndim)     	!wave lenght
        double precision wavedi(nkndim)     	!wave dissipation
	double precision SXX3D(nlvdim,nkndim)	!radiation stress xx
	double precision SYY3D(nlvdim,nkndim)	!radiation stress yy
	double precision SXY3D(nlvdim,nkndim)	!radiation stress xy
	double precision wtau,taux,tauy		!wave and wind stresses
        double precision s,d			!speed and direction
...
! local
	...
	double precision ddl(nlvdim,nkndim)		!3D layer depth (in the middle of layer)
	double precision h(nlvdim)
...
! Computes wave forcing terms according to the radiation stress formulation
	...
	use basin
        ...
        double precision wavefx(nlvdim,neldim)      !wave forcing term x
        double precision wavefy(nlvdim,neldim)      !wave forcing term y

        double precision b,c           !x and y derivated form function [1/m]
	integer k,ie,ii,l,ilevel
        double precision radsx,radsy
...
! arguments
        ...
        double precision wavejb(nkndim)             	!wave pressure
        double precision wavefx(nlvdim,neldim)		!x wave forcing term
	double precision wavefy(nlvdim,neldim)		!y wave forcing term

! common
	include 'pkonst.h'

! local
        double precision stokesz(nlvdim,nkndim)	!z stokes velocity on node k
	double precision hk(nlvdim)			!layer tickness on nodes
	integer k,ie,ii,l,ilevel
	double precision f				!Coriolis parameter on elements
	double precision h				!layer thickness
	double precision u,v			!velocities at level l and elements
	double precision stxe(nlvdim,neldim)	!x stokes transport on elements
	double precision stye(nlvdim,neldim)	!y stokes transport on elements
	double precision stxk, styk			!stokes transport on nodes
	double precision auxx, auxy			!auxiliary variables
	double precision jbk			!integrated wave perssure term
        double precision b,c		!x and y derivated form function [1/m]
	double precision wavesx,wavesy
        double precision saux1(nlvdim,nkndim)
        double precision saux2(nlvdim,nkndim)
	double precision stokesze(0:nlvdim,neldim)	!z stokes velocity on elements
	double precision wuz,wvz			!z vortex force
	double precision sz,sz1
	double precision uaux(0:nlvdim+1),vaux(0:nlvdim+1)
...
! parameters
        include 'param.h'
! arguments
        double precision vf(nlvdim,nkndim)		!auxiliary array
        double precision va(nlvdim,nkndim)		!auxiliary array
	double precision stxe(nlvdim,neldim)	!x stokes transport on elements
	double precision stye(nlvdim,neldim)	!y stokes transport on elements
	double precision auxstz(nlvdim,nkndim) 	!z stokes velocity on node k for plotting
	double precision stokesze(0:nlvdim,neldim)	!z stokes velocity on elements

! common

! local
	double precision stokesz(0:nlvdim,nkndim) 	!z stokes velocity on node k
        logical debug
        integer k,ie,ii,kk,l,lmax
        integer ilevel
        double precision b,c            !x and y derivated form function [1/m]
	double precision aj,ff,atop,acu
...
! --- input variable
        double precision winds(neldim)	!wind speed at 10m [m/s]
        double precision windd(neldim)	!wind direction [degree north]
        double precision fet(neldim)        !wind fetch length [m]
        double precision daf(neldim)        !averaged depth along the fetch [m]

! --- output variable

        double precision, save, allocatable :: waeh(:)	!wave height [m]
        double precision, save, allocatable :: waep(:)	!wave period [s]
        double precision, save, allocatable :: waed(:)	!wave direction (same as wind
!        common /waeh/waeh, /waep/waep, /waed/waed

! --- stress variables
        double precision tcv(neldim)
        double precision twv(neldim)
        double precision tmv(neldim)


! --- local variable
	logical debug
        double precision hbr		!limiting wave height [m]
        double precision dep,depe
        double precision gh,gx,hg
	double precision wis,wid
	double precision wx,wy
	double precision v1v(nkn)
        integer ie,icount,ii,k

        double precision g			!gravity acceleration [m2/s]
        parameter (g=9.81)
        double precision z0
        parameter (z0=5.e-4)
        double precision ah1,ah2,ah3,eh1,eh2,eh3,eh4
        double precision at1,at2,at3,et1,et2,et3,et4
        double precision auxh,auxh1,auxt,auxt1
...
! --- calculate the wave height, period and direction
...
! grid given the wind direction.
	...
	use coordinates
        ...
        double precision xe,ye		!element point coordinates [m]
	double precision fff,ddd
        double precision rad,wdir,wid
...
! grid given the wind direction.
	...
	integer ie
	double precision xein,yein
	double precision wdir
	double precision fff,ddd
	integer ierr

        double precision xe,ye		!element point coordinates [m]
        double precision xnew,ynew		!new coordinates [m]
        double precision de			!distance between points [m]
        double precision dep		!element depth [m]
...
! line and one of the border line of the element
	...
	use coordinates
        ...
        double precision x,y		!start point cooridnates [m]
        double precision wdir		!direction to search [radians]
        integer ien		!next element number (return)
        double precision xn,yn		!intersection coordinates [m] (return)
	integer ieold
	logical bdebug

	include 'param.h'

        double precision x0(3),y0(3)        !element vertices coordinates [m]
        double precision xg0(3),yg0(3)      !element vertices coordinates [degrees]
        double precision x3,y3,x4,y4	!node points coordiantes [m]
        double precision xi,yi		!intersection point coordinate [m]
	double precision xf,yf		!far away point
...
! computes stress parameters
	...
	use basin, only : nkn,nel,ngr,mbw
        ...
        double precision z0
        double precision tcv(1)
        double precision twv(1)
        double precision tmv(1)


        double precision pi,karm,rho,g
        double precision depth,ux,uy,uc2
        double precision h,p
        double precision aux,cd
        double precision omega,zeta,eta
        double precision uw,a,fw
        double precision tc,tw,tm

        integer ie

        double precision waeh(neldim)	!wave height [m]
        double precision waep(neldim)	!wave period [s]
...
! mean wave direction
	...
	include 'param.h'
        ...
        integer k
        double precision wh                 !sign. wave height [m]
        double precision wmp                !mean wave period [s]
        double precision wpp                !peak wave period [s]
        double precision wd                 !mean wave direction [deg]
