...
!----------------------------------------------------------------------------
	...
	integer nkn
	integer ilhkv(nkn)
	double precision cv3(nlvddi,nkn)
	double precision vol3(nlvddi,nkn)
	double precision cv2(nkn)
	...
	do k=1,nkn
	  ...
	  if( bdebug ) write(42,*) lmax,vvtot,cctot,dble(cctot/vvtot)
	  cctot = cctot / vvtot
	  cv2(k) = cctot
	end do
...
!***************************************************************
	...
	subroutine make_basin_aver(nlvddi,nkn,ilhkv,cv3,vol3,cmin,cmax,cmed,vtot)
	...
	double precision cv3(nlvddi,nkn)
	double precision vol3(nlvddi,nkn)
	double precision cmin,cmax,cmed,vtot
...
!***************************************************************
	...
	integer ilhkv(nkn)
	double precision cv3(nlvddi,nkn)
	double precision cvacu(nlvddi,nkn)
...
!***************************************************************
	...
	subroutine init_volume(nlvddi,nkn,nel,nlv,nen3v,ilhkv,hlv,hev,hl,vol3)
...
! we could do better using information on node area and depth structure
	...
	double precision hl(nlv)		!aux vector for layer thickness
	double precision vol3(nlvddi,nkn)

	logical bvolwrite,bdebug
	integer ie,ii,k,l,lmax,nsigma,nlvaux,ks
	double precision z,h,hsigma
...
! reads volumes
	...
	integer nlvddi
	integer ilhkv(1)
	double precision vol3(nlvddi,1)
