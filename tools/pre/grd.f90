!
! $Id: subgrd.f,v 1.9 2009-05-21 09:24:00 georg Exp $
!
...
!==============================================================
	...
	implicit none
        ...
        double precision, save :: xscale_grd = 1.
        double precision, save :: yscale_grd = 1.
        double precision, save :: zscale_grd = 1.
	character*80, save :: title_grd = ' '
	double precision, save :: dcor_grd = 0.
	double precision, save :: dirn_grd = 0.

        integer, save :: nin_grd,iline_grd,ianz_grd
        double precision, save :: f_grd(80)
	...
	integer, save, allocatable :: ianv(:),iaev(:),ialv(:)
        double precision, save, allocatable :: hhnv(:),hhev(:),hhlv(:)
        double precision, save, allocatable :: xv(:),yv(:)
...
!*****************************************************************
	...
	call rdgrd(file,bstop,nco,nk,ne,nl,nne,nnl,nkndi,neldi,nlidi,nendi,nlndi,ippnv,ippev &
     &                  ,ipplv,ianv,iaev,ialv,hhnv,hhev,hhlv,xv,yv,ipntev,inodev,ipntlv,inodlv)
...
! reads grd file to obtain basic parameters
	...
	integer ianv(1),iaev(1),ialv(1)
        double precision hhnv(1),hhev(1),hhlv(1)
        double precision xv(1),yv(1)
...
!-----------------------------------------------------------------
        ...
        call rdgrd(gfile,bstop,nco,nk,ne,nl,nne,nnl,nkndi0,neldi0,nlidi0,nendi0,nlndi0,ippnv &
     &          ,ippev,ipplv,ianv,iaev,ialv,hhnv,hhev,hhlv,xv,yv,ipntev0,inodev,ipntlv0,inodlv)
...
!**********************************************************
	...
	call grd_write_grid(file,nco,nk,ne,nl,nne,nnl,ippnv,ippev,ipplv,ianv &
     &          ,iaev,ialv,hhnv,hhev,hhlv,xv,yv,ipntev,inodev,ipntlv,inodlv)
...
!**********************************************************
	...
	subroutine rdgrd(file,bstop,nco,nkn,nel,nli,nne,nnl,nknddi,nelddi,nliddi,nenddi,nlnddi,ipnv &
     &                  ,ipev,iplv,ianv,iaev,ialv,hnv,hev,hlv,xgv,ygv,ipntev,inodev,ipntlv,inodlv)
...
! works only with triangles as elements
	...
	double precision hnv(nknddi)	!depth of node
	double precision hev(nelddi)	!depth of element
	double precision hlv(nliddi)	!depth of line

	double precision xgv(nknddi)	!x coordinate of node
	double precision ygv(nknddi)	!y coordinate of node
...
!--------------------------------------------------------------------
	  ...
	  iwhat = nint(value)
		...
		call rdcom(nco,bstop)
        	...
        	call rdnode(nkn,nknddi,bstop,ipnv,ianv,xgv,ygv,hnv)
          else if(iwhat.eq.2) then              !element
        	call rdelem(nel,nne,nelddi,nenddi,bstop,ipev,iaev,ipntev,inodev,hev)
          else if(iwhat.eq.3) then              !line
        	call rdline(nli,nnl,nliddi,nlnddi,bstop,iplv,ialv,ipntlv,inodlv,hlv)
          else
...
!**********************************************************
        ...
        subroutine rdnode(nkn,nknddi,bstop,ipnv,ianv,xgv,ygv,hnv)
...
! reads nodes from .grd file
	...
	logical bstop
        integer ipnv(nknddi)
        integer ianv(nknddi)         !node type
	double precision xgv(nknddi),ygv(nknddi)
	double precision hnv(nknddi)
...
!**********************************************************
        ...
        subroutine rdelem(nel,nne,nelddi,nenddi,bstop,ipev,iaev,ipntev,inodev,hev)
...
!**********************************************************
        ...
        subroutine rdline(nli,nnl,nliddi,nlnddi,bstop,iplv,ialv,ipntlv,inodlv,hlv)
...
!*****************************************************************
	...
	subroutine grd_write_grid(file,nco,nk,ne,nl,nne,nnl,ippnv,ippev,ipplv,ianv,iaev,ialv &
     &			        ,hhnv,hhev,hhlv,xv,yv,ipntev,inodev,ipntlv,inodlv)
...
! writes grd file
	...
	double precision hhnv(nk)		!depth of node
	double precision hhev(ne)		!depth of element
	double precision hhlv(nl)		!depth of line

	double precision xv(nk)		!x coordinate of node
	double precision yv(nk)		!y coordinate of node
	...
	do i=1,ne
	  ...
	  ib = ipntev(ie-1)
	  call grd_write_item(nout,2,ieext,ia,n,inodev(ib+1),nextern,depth)
	end do
	...
	do i=1,nl
	  ...
	  ib = ipntlv(il-1)
	  call grd_write_item(nout,3,ilext,ia,n,inodlv(ib+1),nextern,depth)
	end do
...
!*****************************************************************
	...
	subroutine grd_write_item(nout,iwhat,number,itype,n,nodes,extern,depth)
	...
	if( n > 3 ) then
	  ...
	  write(nout,2000) iwhat,number,itype,n,(extern(nodes(i)),i=1,n)
	else
	  write(nout,2000) iwhat,number,itype,n,(extern(nodes(i)),i=1,n),depth
	end if
...
!*****************************************************************
	...
	integer nk,ne
	double precision hkv(nk)
	double precision hev(ne)
